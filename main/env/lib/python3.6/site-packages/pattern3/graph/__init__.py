...
# If you want to call Graph.draw() then line(), ellipse() and Text.draw()
# must be implemented.
...
class Edge(object):
    ...
    def _set_weight(self, v):
        self._weight = v
        # Clear cached adjacency map in the graph, since edge weights have
        # changed.
...
class Graph(dict):
    ...
    def __init__(self, layout=SPRING, distance=10.0):
        ...
        self.layout = layout == SPRING and GraphSpringLayout(
            self) or GraphLayout(self)
    ...
    def add_edge(self, id1, id2, *args, **kwargs):
        ...
        # If an edge (in the same direction) already exists, yields that edge
        # instead.
        e1 = n1.links.edge(n2)
    ...
    def shortest_path(self, node1, node2, heuristic=None, directed=False):
        ...
        if not isinstance(node2, Node):
            node2 = self[node2]
        try:
            p = dijkstra_shortest_path(
                self, node1.id, node2.id, heuristic, directed)
    ...
    def fringe(self, depth=0, traversable=lambda node, edge: True):
        ...
        """
        u = []
        [u.extend(n.flatten(depth, traversable))
         for n in self.nodes if len(n.links) == 1]
        return unique(u)
    ...
    def _add_node_copy(self, n, **kwargs):
        ...
        try:
            new = self.add_node(
                n.id, root=kwargs.get("root", False), text=False)
        except TypeError:
...
# applies topology and geometry to derive two-dimensional representations
# of graphs.
...
class GraphSpringLayout(GraphLayout):
    ...
    def _distance(self, node1, node2):
        ...
        # Ensures that the distance is never zero (which deadlocks the
        # animation).
    ...
    def update(self, weight=10.0, limit=0.5):
        ...
        for e in self.graph.edges:
            self._attract(
                e.node1, e.node2, weight * e.weight, 1.0 / (e.length or 0.01))
    ...
    while q:
        ...
        if not node.id in _visited:
            if visit(node):
                return True
            q.extend(
                (n for n in node.links if traversable(node, node.links.edge(n)) is not False))
            _visited[node.id] = True
    return False
...
def adjacency(graph, directed=False, reversed=False, stochastic=False, heuristic=None):
    ...
    for e in graph.edges:
        id1, id2 = not reversed and (
            e.node1.id, e.node2.id) or (e.node2.id, e.node1.id)
    ...
    if stochastic:
        for id1 in map:
            ...
            for id2 in map[id1]:
                map[id1][id2] /= n
    # Cache the adjacency map: this makes dijkstra_shortest_path() 2x faster
    # in repeated use.
    graph._adjacency = (
        map, directed, reversed, stochastic, heuristic and heuristic.__code__)
    return map
...
def eigenvector_centrality(graph, normalized=True, reversed=True, rating={}, iterations=100, tolerance=0.0001):
    ...
    # http://python-networkx.sourcearchive.com/documentation/1.0.1/centrality_8py-source.html
    # Note: much faster than betweenness centrality (which grows
    # exponentially).
    ...
    v = normalize(dict([(n, random()) for n in graph]))
...
def clique(graph, id):
    ...
    for n in graph.nodes:
        try:
            # Raises StopIteration if all nodes in the clique are connected to
            # n:
...
class HTMLCanvasRenderer(GraphRenderer):
    ...
    def __init__(self, graph, **kwargs):
        ...
        # Either None, INLINE, DEFAULT (style.css) or a custom path.
        ...
        # Shortens leaf edges, adds eigenvector weight to node radius.
        self.pack = True
        # JS GraphLayout
    ...
    def _data(self):
        ...
        for n in self.graph.nodes:
            ...
            if n.text and "font" in n.text.__dict__:
                # "sans-serif"
                p.append("font:\"%s\"" % n.text.__dict__["font"])
...
class GraphMLRenderer(GraphRenderer):
    ...
    def export(self, path, directed=False, encoding="utf-8"):
        ...
        m = {}
        g = etree.SubElement(
            root, ns + "graph", id="g", edgedefault=directed and "directed" or "undirected")
        # Export nodes.
        ...
        for i, e in enumerate(self.graph.edges):
            x = etree.SubElement(
                g, ns + "edge", id="edge%s" % i, source=m[e.node1.id], target=m[e.node2.id])
...
# and are expected to handle any GraphRenderer by specifying an optional
# type=HTML|GRAPHML.
