...
def filter_blissnet():
    ...
    for uni in bn:
        ...
        meanings = UNICODE_TO_BLISS[uni]
        meaning = meanings[0]
        changed = False

        print "The Blissymbol meaning", meaning, "is associated with the following synset(s):"
        ...
        for synset in synsets:
            ...
            print
            print "Does the Blissymbol for " + meaning + " match the synset " + names[0] + "?"
            ans = raw_input("press enter for yes, enter any key for no\t")
            print
            if len(ans) != 0:
                synsets_set.remove(synset)
                changed = True


        if changed:
            BLISSNET[uni] = list(synsets)
        print
        break
...
def write_blissnet():
    ...
    with open("blissnet_out.txt", "w") as out:
        tab = "    "
        out.write("BLISSNET = {\n")

        for uni in sorted(bn):
            meanings = UNICODE_TO_BLISS[uni]
            comment = "# " + ", ".join(meanings)
            out.write(tab + comment + "\n")
            out.write(('%s"' % tab) + uni + '":    [')
            synsets = bn[uni]
            for synset in synsets:
                out.write('wn.synset("' + synset.name() + ('"),\n%s  ' % (tab*4)))
            out.write('],\n\n')
