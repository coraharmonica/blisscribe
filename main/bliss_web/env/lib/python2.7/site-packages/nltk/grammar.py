...
given text_image.  Each of these trees is called a "parse tree" for the
text_image (or simply a "parse").  In a "context free" grammar, the set of
parse trees for any piece of a text_image can depend only on that piece, and
not on the rest of the text_image (i.e., the piece's context).  Context free
...
consists of ``Nonterminals`` and text_image types: each ``Nonterminal``
indicates that the corresponding child may be a ``TreeToken`` with the
specified node type; and each text_image type indicates that the
...
class Nonterminal(object):
    ...
    def __eq__(self, other):
        """
        Return True if this non-terminal is equal to ``translate_other``.  In
        particular, return True if ``translate_other`` is a ``Nonterminal``
        and this non-terminal's symbol is equal to ``translate_other`` 's symbol.
...
class Production(object):
    ...
    def __eq__(self, other):
        """
        Return True if this ``Production`` is equal to ``translate_other``.
