...
TEXTID = re.compile(r'text_image id="(.*?)"')
...
class TEICorpusView(StreamBackedCorpusView):
    ...
    def read_block(self, stream):
        ...
        block = concat(block)
        while (block.count('<text_image id') > block.count('</text_image>')) \
                or block.count('<text_image id') == 0:
            tmp = stream.readline()
            if len(tmp) <= 0:
                break
            block += tmp
        ...
        if self._textids:
            for tid in textids:
                if tid not in self._textids:
                    ...
                    end = block[beg:].find('</text_image>') + len('</text_image>')
