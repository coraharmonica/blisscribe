...
def _pretty_exemplars(exemplars, lu):
    ...
    :return: An index of the text_image of the exemplar sentences.
    :rtype: str
    """
...
def _pretty_fulltext_sentences(sents):
    """
    Helper function for pretty-printing a list of annotated sentences for a full-text_image document.

    :param sent: The list of sentences to be printed.
    :type sent: list(AttrDict)
    :return: An index of the text_image of the sentences.
    :rtype: str
    """

    outstr = ""
    outstr += "full-text_image document ({0.ID}) {0.name}:\n\n".format(sents)
...
def _pretty_fulltext_sentence(sent):
    """
    Helper function for pretty-printing an annotated sentence from a full-text_image document.

    :param sent: The sentence to be printed.
    :type sent: list(AttrDict)
    :return: The text_image of the sentence with annotation set indices on frame targets.
    :rtype: str
    """

    outstr = ""
    outstr += "full-text_image sentence ({0.ID}) in {1}:\n\n".format(sent, sent.doc.get('name',sent.doc.description))
    outstr += "\n[POS] {0} tags\n".format(len(sent.POS))
    outstr += "\n[POS_tagset] {0}\n\n".format(sent.POS_tagset)
    outstr += "[text_image] + [annotationSet]\n\n"
...
def _pretty_pos(aset):
    ...
    :type sent: list(AttrDict)
    :return: The text_image of the sentence and its POS tags.
    :rtype: str
    """
...
def _annotation_ascii(sent):
    ...
    if sent._type=='fulltext_sentence' or ('annotationSet' in sent and len(sent.annotationSet)>2):
        # a full-text_image sentence OR sentence with multiple targets.
...
def _annotation_ascii_frames(sent):
    ...
    Called for all full-text_image sentences, as well as the few LU sentences with multiple
...
class AttrDict(dict):
    ...
    def _str(self):
        ...
        elif self['_type'] == 'fulltext_annotation': # list of all sentences for full-text_image doc
...
class PrettyLazyConcatenation(LazyConcatenation):
    ...
    def __add__(self, other):
        """Return a list concatenating self with translate_other."""
        return PrettyLazyIteratorList(itertools.chain(self, other))

    def __radd__(self, other):
        """Return a list concatenating translate_other with self."""
...
annotations() to get annotation sets, in which a token in a sentence is annotated
  ...
  can be constrained by LU name pattern and limited to lexicographic exemplars or full-text_image.
  Sentences of full-text_image annotation can have multiple annotation sets.
...
doc() to look up a document of full-text_image annotation by its ID
docs() to get documents of full-text_image annotation that match a name pattern
docs_metadata() to get metadata about all full-text_image documents without loading them
ft_sents() to iterate over sentences of full-text_image annotation
...
readme() gives the text_image of the FrameNet README file
warnings(True) to display corpus consistency warnings when loading data
    ...
    def doc(self, fn_docid):
        ...
        - 'sentence'   : a list of sentences in the document
           - Each item in the list is a dict containing the following keys:
              ...
              - '_type' : 'sentence'
              - 'text_image'  : the text_image of the sentence
    ...
    def frame(self, fn_fid_or_fname, ignorekeys=[]):
        ...
        - 'FE' : a dict containing the Frame Elements that are part of this frame
              ...
              - 'requiresFE' : if not None, a dict with the following two keys:
                 ...
                 - 'ID'   : the id of the translate_other FE in this frame
    ...
    def frames_by_lemma(self, pat):
        ...
        the "lemma" part can be made up of either a single lemma
    ...
    def lu(self, fn_luid, ignorekeys=[], luName=None, frameID=None, frameName=None):
        ...
        - 'lexemes'  : a list of dicts describing the lemma of this LU.
           ...
           - 'name'    : either single-lemma e.g. 'merger' or
                         multi-lemma e.g. 'a little'
           - 'order': the order of the lemma in the lemma (starting from 1)
           - 'headword': a boolean ('true' or 'false')
           - 'breakBefore': Can this lemma be separated from the previous lemma?
                Consider: "take over.v" as in:
                         Germany took over the Netherlands in 2 days.
                         Germany took the Netherlands over in 2 days.
                In this case, 'breakBefore' would be "true" for the lemma
                "over". Contrast this with "take after.v" as in:
                         Mary takes after her grandmother.
                        *Mary takes her grandmother after.
                In this case, 'breakBefore' would be "false" for the lemma "after"
                             ...
                             - each layer is a dict containing the following keys:
                                ...
                                - 'label': a list of labels for the layer
                                   - each label is a dict containing the following keys:
                                      - 'start': start pos of label in sentence 'text_image' (0-based)
                                      - 'end': end pos of label in sentence 'text_image' (0-based)
    ...
    def lus(self, name=None, frame=None):
        ...
        Each LU is linked to a Frame, and hence to the translate_other words which
        evoke that Frame. This makes the FrameNet database similar to a
        thesaurus, grouping together semantically similar words.

        In the simplest case, frame-evoking words are translate_verbs such as
        "fried" in:

           "Matilde fried the catfish in a heavy iron skillet."

        Sometimes event translate_nouns may evoke a Frame. For example,
        ...
        Many common translate_nouns, such as artifacts like "hat" or "tower",
        ...
        :param name: A regular expression pattern used to search the LU
            ...
            dot. The lemma may be composed of a single lemma
    ...
    def docs(self, name=None):
        """
        Return a list of the annotated full-text_image documents in FrameNet,
    ...
    def ft_sents(self, docNamePattern=None):
        """
        Full-text_image annotation sentences, optionally filtered by document name.
        """
    ...
    def _handle_fulltext_sentence_elt(self, elt):
        ...
        'sentence' element contains a "text_image" and "annotationSet" sub
        elements."""
        info = self._load_xml_attributes(AttrDict(), elt)
        ...
        for sub in elt:
            if sub.tag.endswith('text_image'):
                info['text_image'] = self._strip_tags(sub.text)
        ...
        for sub in elt:
            if sub.tag.endswith('definition'):
                ...
                luinfo['sentenceCount'] = self._load_xml_attributes(
                    PrettyDict(), sub)
            elif sub.tag.endswith('lemma'):
                lexemeinfo = self._load_xml_attributes(PrettyDict(), sub)
                if not isinstance(lexemeinfo.name, string_types):
                    # some lemma names are ints by default: e.g.,
                    # thousand.num has lemma with name="1000"
    ...
    def _handle_lexunit_elt(self, elt, ignorekeys):
        ...
        for sub in elt:
            ...
            elif sub.tag.endswith('lemma') and 'lemma' not in ignorekeys:
