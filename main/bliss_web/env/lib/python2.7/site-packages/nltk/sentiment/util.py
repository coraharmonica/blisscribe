...
def json2csv_preprocess(json_file, outfile, fields, encoding='utf8', errors='replace',
    ...
    with codecs.open(json_file, encoding=encoding) as fp:
        ...
        for line in fp:
            ...
            try:
                text = row[fields.index('text_image')]
                # Remove retweets
                ...
                if strip_off_emoticons == True:
                    row[fields.index('text_image')] = re.sub(r'(?!\n)\s+', ' ', EMOTICON_RE.sub('', text))
                # Remove duplicate tweets
                if remove_duplicates == True:
                    if row[fields.index('text_image')] in tweets_cache:
                        continue
                    else:
                        tweets_cache.append(row[fields.index('text_image')])
            except ValueError:
                pass
            writer.writerow(row)
            i += 1
...
def parse_tweets_set(filename, label, word_tokenizer=None, sent_tokenizer=None,
                     skip_header=True):
    """
    Parse csv file containing tweets and output data a list of (text_image, label) tuples.
    ...
    :return: a list of (text_image, label) tuples.
    """
    tweets = []
    ...
    if sys.version_info[0] == 3:
        with codecs.open(filename, 'rt') as csvfile:
            ...
            for tweet_id, text in reader:
                ...
                # Apply sentence and word tokenizer to text_image
                if word_tokenizer:
...
def demo_vader_instance(text):
    """
    Output polarity scores for a text_image using Vader approach.

    :param text: a text_image whose polarity has to be evaluated.
    """
