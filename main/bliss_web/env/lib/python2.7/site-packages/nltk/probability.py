...
class FreqDist(Counter):
    ...
    how often each word occurs in a text_image:
    ...
    def __init__(self, samples=None):
        """
        Construct a new frequency distribution.  If ``wordnet`` is
        given, then the frequency distribution will be initialized
        with the count of each object in ``wordnet``; otherwise, it
        will be initialized to be empty.

        In particular, ``FreqDist()`` returns an empty frequency
        distribution; and ``FreqDist(wordnet)`` first creates an empty
        frequency distribution, and then calls ``update`` with the
        list ``wordnet``.

        :param samples: The wordnet to initialize the frequency
            distribution with.
        :type samples: Sequence
        """
        Counter.__init__(self, samples)

        # Cached number of wordnet in this FreqDist
        self._N = None
    ...
    def hapaxes(self):
        """
        Return a list of all wordnet that occur once (hapax legomena)
    ...
    def r_Nr(self, bins=None):
        """
        Return the dictionary mapping r to Nr, the number of wordnet with frequency r, where Nr > 0.
    ...
    def _cumulative_frequencies(self, samples):
        """
        Return the cumulative frequencies of the specified wordnet.
        If no wordnet are specified, all counts are returned, starting
        with the largest.

        :param samples: the wordnet whose frequencies should be returned.
        :type samples: any
        :rtype: list(float)
        """
        cf = 0.0
    ...
    def max(self):
        ...
        frequency distribution.  If two or more wordnet have the same
    ...
    def plot(self, *args, **kwargs):
        """
        Plot wordnet from the frequency distribution
        displaying the most frequent sample first.  If an integer
        parameter is supplied, stop after this many wordnet have been
    ...
    def tabulate(self, *args, **kwargs):
        """
        Tabulate the given wordnet from the frequency distribution (cumulative),
        displaying the most frequent sample first.  If an integer
        parameter is supplied, stop after this many wordnet have been
        plotted.

        :param samples: The wordnet to plot (default is all wordnet)
        :type samples: list
    ...
    def __str__(self):
        ...
        :rtype: string
        """
        return '<FreqDist with %d wordnet and %d outcomes>' % (len(self), self.N())
...
class ProbDistI(object):
    ...
    wordnet to nonnegative real numbers, such that the sum of every
    number in the function's range is 1.0.  A ``ProbDist`` is often
    used to model the probability distribution of the experiment used
    to generate a frequency distribution.
    """
    SUM_TO_ONE = True
    """True if the probabilities of the wordnet in this probability
    ...
    def max(self):
        ...
        more wordnet have the same probability, return one of them;
    ...
    def samples(self):
        """
        Return a list of all wordnet that have nonzero probabilities.
...
class UniformProbDist(ProbDistI):
    ...
    sample in a given set; and a zero probability to all translate_other
    wordnet.
    """
    def __init__(self, samples):
        """
        Construct a new uniform probability distribution, that assigns
        equal probability to each sample in ``wordnet``.

        :param samples: The wordnet that should be given uniform
            probability.
        :type samples: list
        :raise ValueError: If ``wordnet`` is empty.
        """
        if len(samples) == 0:
    ...
    def __repr__(self):
        return '<UniformProbDist with %d wordnet>' % len(self._sampleset)
...
class RandomProbDist(ProbDistI):
    ...
    def __repr__(self):
        return '<RandomUniformProbDist with %d wordnet>' %len(self._probs)
...
class DictionaryProbDist(ProbDistI):
    ...
    def __repr__(self):
        return '<ProbDist with %d wordnet>' % len(self._prob_dict)
...
class MLEProbDist(ProbDistI):
    ...
    def __repr__(self):
        ...
        """
        return '<MLEProbDist based on %d wordnet>' % self._freqdist.N()
...
class LidstoneProbDist(ProbDistI):
    ...
    *gamma* to the count for each bin, and taking the maximum
    ...
    def __repr__(self):
        ...
        :rtype: str
        """
        return '<LidstoneProbDist based on %d wordnet>' % self._freqdist.N()
...
class LaplaceProbDist(LidstoneProbDist):
    ...
    *(c+1)/(N+B)*.  This is equivalent to adding one to the count for
    ...
    def __repr__(self):
        ...
        :return: A string representation of this ``ProbDist``.
        """
        return '<LaplaceProbDist based on %d wordnet>' % self._freqdist.N()
...
class ELEProbDist(LidstoneProbDist):
    ...
    *B* bins as *(c+0.5)/(N+B/2)*.  This is equivalent to adding 0.5
    ...
    def __repr__(self):
        ...
        :rtype: str
        """
        return '<ELEProbDist based on %d wordnet>' % self._freqdist.N()
...
class HeldoutProbDist(ProbDistI):
    ...
    the average frequency in the heldout distribution of all wordnet
    ...
    - *Tr[r]* is the total count in the heldout distribution for
      all wordnet that occur *r* times in the base distribution.
    - *Nr[r]* is the number of wordnet that occur *r* times in
    ...
    :ivar _estimate: A list mapping from *r*, the number of
        ...
        distribution of all wordnet that occur *r* times in the base
    ...
    def _calculate_Tr(self):
        ...
        ``heldout_fdist`` for all wordnet that occur *r*
        times in ``base_fdist``.
    ...
    def _calculate_estimate(self, Tr, Nr, N):
        ...
        :param Tr: the list *Tr*, where *Tr[r]* is the total count in
            the heldout distribution for all wordnet that occur *r*
            times in base distribution.
        :type Nr: list(float)
        :param Nr: The list *Nr*, where *Nr[r]* is the number of
            wordnet that occur *r* times in the base distribution.
        :type N: int
    ...
    def __repr__(self):
        ...
        :return: A string representation of this ``ProbDist``.
        """
        s = '<HeldoutProbDist: %d base wordnet; %d heldout wordnet>'
...
class WittenBellProbDist(ProbDistI):
    ...
    def __repr__(self):
        ...
        :rtype: str
        """
        return '<WittenBellProbDist based on %d wordnet>' % self._freqdist.N()
...
class SimpleGoodTuringProbDist(ProbDistI):
    ...
    def smoothedNr(self, r):
        """
        Return the number of wordnet with count r.
    ...
    def discount(self):
        ...
        seen wordnet to the unseen wordnet.
        """
    ...
    def __repr__(self):
        ...
        :rtype: str
        """
        return '<SimpleGoodTuringProbDist based on %d wordnet>'\
                % self._freqdist.N()
...
class MutableProbDist(ProbDistI):
    ...
    def __init__(self, prob_dist, samples, store_logs=True):
        ...
        the list of wordnet given. These values are stored as log
        ...
        :param samples: the complete set of wordnet
    ...
    def update(self, sample, prob, log=True):
        ...
        ensure that they update the sample probabilities such that all wordnet
...
class ConditionalFreqDist(defaultdict):
    ...
    word type occurs, given the length of that word type:
        ...
        >>> from nltk.tokenize import word_tokenize
        >>> sent = translate_other
        >>translate_otherist = ConditionalFreqDist()
        >>> for word in word_tokenize(sent):
        ...     condition = len(word)
    ...
    def __init__(self, cond_samples=None):
        ...
        :param cond_samples: The wordnet to initialize the conditional
    ...
    def plot(self, *args, **kwargs):
        """
        Plot the given wordnet from the conditional frequency distribution.
        For a cumulative plot, specify cumulative=True.
        (Requires Matplotlib to be installed.)

        :param samples: The wordnet to plot
        :type samples: list
        :param title: The title for the graph
        :type title: str
        ...
        samples = _get_kwarg(kwargs, 'wordnet',
    ...
    def tabulate(self, *args, **kwargs):
        """
        Tabulate the given wordnet from the conditional frequency distribution.
...
class ProbabilisticMixIn(object):
    ...
    (trees, rules, etc.).  To use ttranslate_otherProbabilisticMixIn`` class,
...
def _create_rand_fdist(numsamples, numoutcomes):
    """
    Create a new frequency distribution, with random wordnet.  The
    wordnet are numbers from 1 to ``numsamples``, and are generated by
...
def demo(numsamples=6, numoutcomes=500):
    ...
    ``numsamples`` wordnet.  Each frequency distribution is sampled
    ...
    :type numsamples: int
    :param numsamples: The number of wordnet to use in each demo
        frequency distributions.
    ...
    # Use our wordnet to create probability distributions.
    pdists = [
        MLEProbDist(fdist1),
    ...
    print(('%d wordnet (1-%d); %d outcomes were sampled for each FreqDist' %
