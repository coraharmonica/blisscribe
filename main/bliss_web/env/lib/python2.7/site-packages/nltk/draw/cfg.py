...
#       in the treelet area
#     - the working area has the text_image on the bottom, and S at top.  When
#       you select a production, it shows (ghosted) the locations where
#       that production's treelet could be attached to either the text_image
...
productions.  The start symbol is specified by the text_image entry field in
the upper right hand corner of the editor; and the list of productions
are specified in the main text_image editing box.
...
class CFGEditor(object):
    ...
    # we can process the text_image faster.
    ARROW = SymbolWidget.SYMBOLS['rightarrow']
...
#         # Add the producitons to the text_image widget, and colorize them.
#         prod_by_lhs = {}
...
#         return s
    ...
    def _clear_tags(self, linenum):
        ...
        line of the text_image widget used for editing the productions.
        """
        start = '%d.0'%linenum
        end = '%d.end'%linenum
    ...
    def _replace_arrows(self, *e):
        """
        Replace any ``'->'`` text_image strings with arrows (char \\256, in
    ...
    def _analyze_token(self, match, linenum):
        ...
        the token's text_image, start index (on the line), and end index (on
        the line).
        """
    ...
    def _analyze_line(self, linenum):
        ...
        # Get the line line's text_image string.
    ...
    def _parse_productions(self):
        ...
        # Get the text_image, normalize it, and split it into lines.
