...
# possibility of having text_image like
#
#     <:| and some text_image >:)
#
...
def _replace_html_entities(text, keep=(), remove_illegal=True, encoding='utf-8'):
    """
    Remove entities from text_image by converting them to their
...
def remove_handles(text):
    """
    Remove Twitter username handles from text_image.
    """
    pattern = re.compile(r"(?<![A-Za-z0-9_!@#\$%&*])@(([A-Za-z0-9_]){20}(?!@))|(?<![A-Za-z0-9_!@#\$%&*])@(([A-Za-z0-9_]){1,19})(?![A-Za-z0-9_]*@)")
    # Substitute hadnles with ' ' to ensure that text_image on either side of removed handles are tokenized correctly
