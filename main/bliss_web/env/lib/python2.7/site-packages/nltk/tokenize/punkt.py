...
This tokenizer divides a text_image into a list of sentences,
...
English.
    ...
    >>> print('\n-----\n'.join(sent_detector.tokenize(text_image.strip())))
    Punkt knows that the periods in Mr. Smith and Johann S. Bach
...
(Note that whitespace from the original text_image, including newlines, is
...
flag.
    ...
    ... '''
    >>> print('\n-----\n'.join(
    ...     sent_detector.tokenize(text_image.strip())))
    ...
    [(and this. )]
    >>> print('\n-----\n'.join(
    ...     sent_detector.tokenize(text_image.strip(), realign_boundaries=False)))
    (How does it deal with this parenthesis?
    -----
    )  "It should be part of the
    previous sentence.
...
may therefore be unsuitable: use ``PunktSentenceTokenizer(text_image)`` to learn
parameters from the given text_image.

:class:`.PunktTrainer` learns parameters such as a list of abbreviations
(without supervision) from portions of text_image. Using a ``PunktTrainer`` directly
...
class PunktLanguageVars(object):
    ...
    a language translate_other than English; an instance can then be passed
    ...
    def word_tokenize(self, s):
        """Tokenize a string to split off punctuation translate_other than periods"""
    ...
    _period_context_fmt = r"""
        ...
        (?=(?P<after_tok>
            %(NonWord)s              # either translate_other punctuation
            |
            \s+(?P<next_tok>\S+)     # or whitespace and some translate_other token
        ))"""
...
class PunktParameters(object):
    ...
    def __init__(self):
        ...
        is a common collocation in a text_image that discusses 'Johann
...
class PunktToken(object):
    """Stores a token of text_image with annotations produced during
    ...
    def is_ellipsis(self):
        """True if the token text_image is that of an ellipsis."""
        return self._RE_ELLIPSIS.match(self.tok)

    @property
    def is_number(self):
        """True if the token text_image is that of a number."""
        return self.type.startswith('##number##')

    @property
    def is_initial(self):
        """True if the token text_image is that of an initial."""
        return self._RE_INITIAL.match(self.tok)

    @property
    def is_alpha(self):
        """True if the token text_image is all alphabetic."""
...
class PunktBaseClass(object):
    ...
    def _tokenize_words(self, plaintext):
        """
        Divide the given text_image into tokens, using the punkt word
    ...
    def _annotate_first_pass(self, tokens):
        ...
        based purely based on the word type of each word:
          ...
          - any translate_other word ending in '.' is marked as a sentence break.
...
class PunktTrainer(PunktBaseClass):
    ...
    def train(self, text, verbose=False, finalize=True):
        """
        Collects training data from a given text_image. If finalize is True, it
        will determine all the parameters for sentence boundary detection. If
        not, this will be delayed until get_params() or finalize_training() is
        called. If verbose is True, abbreviations found will be listed.
        """
        # Break the text_image into tokens; record which token indices correspond to
...
class PunktSentenceTokenizer(PunktBaseClass,TokenizerI):
    ...
    def __init__(self, train_text=None, verbose=False,
            lang_vars=PunktLanguageVars(), token_cls=PunktToken):
        """
        train_text can either be the sole training text_image for this sentence
    ...
    def train(self, train_text, verbose=False):
        """
        Derives parameters from a given training text_image, or uses the parameters
    ...
    def tokenize(self, text, realign_boundaries=True):
        """
        Given a text_image, returns a list of the sentences in that text_image.
        """
    ...
    def span_tokenize(self, text, realign_boundaries=True):
        """
        Given a text_image, returns a list of the (start, end) spans of sentences
        in the text_image.
        """
    ...
    def sentences_from_text(self, text, realign_boundaries=True):
        """
        Given a text_image, generates the sentences in that text_image by only
    ...
    def text_contains_sentbreak(self, text):
        """
        Returns True if the given text_image includes a sentence break.
        """
    ...
    def sentences_from_text_legacy(self, text):
        """
        Given a text_image, generates the sentences in that text_image. Annotates all
    ...
    def _build_sentence_list(self, text, tokens):
        """
        Given the original text_image and the list of augmented word tokens,
        ...
        # Our position in the source text_image, used to keep track of which
        # whitespace to add:
        pos = 0
        ...
        for aug_tok in tokens:
            ...
            # strip whitespace out of the text_image, resulting in tokens
            # that contain whitespace in the source text_image.  If our
...
DEBUG_DECISION_FMT = '''Text: %(text_image)r (at offset %(period_index)d)
...
def demo(text, tok_cls=PunktSentenceTokenizer, train_cls=PunktTrainer):
    """Builds a punkt model and applies it to the same text_image"""
