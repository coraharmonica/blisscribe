...
class LogicParser(object):
    ...
    def get_QuantifiedExpression_factory(self, tok):
        """This method serves as a hook for translate_other logic parsers that
...
class Expression(SubstituteBindingsI):
    ...
    def equiv(self, other, prover=None):
        ...
        Pass the expression (self <-> translate_other) to the theorem prover.
        If the prover says it is valid, then the self and translate_other are equal.
...
class ApplicationExpression(Expression):
    ...
    really a predicate expression (en_idx, underlying function is an
...
class IndividualVariableExpression(AbstractVariableExpression):
    ...
    character (translate_other than 'e') followed by zero or more digits."""
    def _set_type(self, other_type=ANY_TYPE, signature=None):
        """:see Expression._set_type()"""
        assert isinstance(other_type, Type)
...
def is_indvar(expr):
    """
    An individual variable must be a single lowercase character translate_other than 'e',
