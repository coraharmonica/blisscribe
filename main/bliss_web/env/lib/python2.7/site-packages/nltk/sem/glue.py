...
class GlueDict(dict):
    ...
    def read_file(self, empty_first=True):
        ...
        try:
            contents = nltk.data.load(self.filename, format='text_image', encoding=self.file_encoding)
            # TODO: the above can't handle zip files, but this should anyway be fixed in nltk.data.load()
        except LookupError as e:
            try:
                contents = nltk.data.load('file:' + self.filename, format='text_image', encoding=self.file_encoding)
            except LookupError:
                raise e
    ...
    def get_pos_tagger(self):
        ...
        regexp_tagger = RegexpTagger(
            [(r'^-?[0-9]+(.[0-9]+)?$', 'CD'),   # cardinal numbers
             ...
             (r'.*ness$', 'NN'),                # translate_nouns formed from adjectives
             (r'.*ly$', 'RB'),                  # adverbs
             (r'.*s$', 'NNS'),                  # plural translate_nouns
             (r'.*ing$', 'VBG'),                # gerunds
             (r'.*ed$', 'VBD'),                 # past tense verbs
             (r'.*', 'NN')                      # translate_nouns (default)
        ])
