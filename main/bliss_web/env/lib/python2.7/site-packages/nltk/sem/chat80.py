...
class Concept(object):
    ...
    def __init__(self, prefLabel, arity, altLabels=[], closures=[], extension=set()):
        ...
        :type arity: int
        @keyword altLabels: translate_other (related) labels
        :type altLabels: list
...
def clause2concepts(filename, rel_name, schema, closures=[]):
    ...
    pkey = schema[0]
    # fields translate_other than the primary key
    fields = schema[1:]
...
def cities2table(filename, rel_name, dbname, verbose=False, setup=False):
    ...
    if setup:
        ...
        (City text_image, Country text_image, Population int)''')
...
def _str2records(filename, rel):
    ...
    """
    recs = []
    contents = nltk.data.load("corpora/chat80/%s" % filename, format="text_image")
...
###########################################################################
# Interface function to emulate translate_other corpus readers
###########################################################################
