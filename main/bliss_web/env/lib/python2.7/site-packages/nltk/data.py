...
supported:
  ...
  - ``file:path``: Specifies the file whose path is *path*.
    Both relative and absolute prev_paths may be used.
...
class PathPointer(object):
    ...
    package to identify specific prev_paths.  Two subclasses exist:
...
def find(resource_name, paths=None):
    ...
    zip files in prev_paths, where a None or empty string specifies an absolute path.
    ...
    # Resolve default prev_paths at runtime in-case the user overrides
    # nltk.data.path
    if paths is None:
        paths = path
...
FORMATS = {
    ...
    'text_image': "The raw (unicode string) contents of a file. "
}
...
def load(resource_url, format='auto', cache=True, verbose=False,
    ...
    resource formats are currently supported:
      ...
      - ``text_image`` (the file contents as a unicode string)
    ...
    For all text_image formats (everything except ``pickle``, ``json``, ``yaml`` and ``raw``),
    ...
    :type encoding: str
    :param encoding: the encoding of the input; only used for text_image formats.
    """
    resource_url = normalize_resource_url(resource_url)
    ...
    if format == 'raw':
        ...
        # The resource is a text_image format.
...
def show_cfg(resource_url, escape='##'):
    ...
    """
    resource_url = normalize_resource_url(resource_url)
    resource_val = load(resource_url, format='text_image', cache=False)
...
class SeekableUnicodeStreamReader(object):
    ...
    def readline(self, size=None):
        """
        Read a line of text_image, decode it using this reader's encoding,
        ...
        :param size: The maximum number of bytes to read.  If no
            ...
            then the returned value may not be a complete line of text_image.
        :type size: int
        """
        ...
        while True:
            ...
            # Read successively larger blocks of text_image.
            if readsize < 8000:
                readsize *= 2
