...
other pieces of text_image).  A list of identifiers for these files is
...
corpora support methods to read the corpus as raw text_image, a list of
...
Indian text_image annotated with part-of-speech tags:
...
in a given item as `IEERDocument` objects:
    ...
    >>> print(docs[0].text_image) # doctest: +ELLIPSIS
    (DOCUMENT
      (LOCATION NAIROBI)
      ,
      (LOCATION Kenya)
...
lists.  These are accessed just like text_image corpora.  The following
...
In the context of a text_image categorization system, we can easily test if the
...
as XML files.  These corpora are returned as ElementTree objects:
    ...
    <Element 'PLAY' at ...>
    >>> print('%s: %s' % (play[0].tag, play[0].text_image))
    TITLE: A Midsummer Night's Dream
    >>> personae = [persona.text_image for persona in
    ...
    >>> speakers = set(speaker.text_image for speaker in
...
.. doctest: +SKIP
    ...
    ...     lexeme = record.find('lx').text_image
    ...     if re.match(r'(.*)\1$', lexeme):
    ...         redup_pos_freqdist[record.find('ps').text_image] += 1
...
This example displays some records from a Rotokas text_image:
...
.. doctest: +SKIP
    ...
    ...     for piece in record:
    ...         if len(piece.text_image) > 60:
    ...             print('%-6s %s...' % (piece.tag, piece.text_image[:57]))
    ...         else:
    ...             print('%-6s %s' % (piece.tag, piece.text_image))
    ref    Paragraph 1
...
is more practical to focus just on the text_image field of the Tweets, which
...
The default tokenizer for Tweets is specialised for 'casual' text_image, and
...
list of XML-formatted 'text_image'/'hypothesis' pairs.
...
whether or not the text_image 'entails' the hypothesis; the
entailment value is mapped to an integer 1 (True) or 0 (False).

    >>> rtepairs[5]
    <RTEPair: gid=2-23>
    >>> rtepairs[5].text_image # doctest: +NORMALIZE_WHITESPACE
...
and ``p.attrib['user']``.  They also have text_image content, accessed using ``p.text_image``.
...
In addition to the above methods for accessing tagged text_image, we can navigate
...
of unannotated text_image files, and the `BracketParseCorpusReader`, which
...
- A *token corpus* contains information about specific occurences of
  ...
  texts.  Examples of token corpora are collections of written text_image
...
corpus, ``fileids()`` will return a list including text_image files, word
...
The ``readline()`` method may read more than a single line of text_image, in
which case it stores the text_image that it does not return in a buffer.  If
...
bytes from the stream:
    ...
    >>> reader.seek(0)            # rewind to the start.
    >>> reader.readline()         # stores extra text_image in a buffer
    u'This is a test file.\n'
