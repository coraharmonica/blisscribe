...
to get the raw contents of any file, simply use ``format="raw"``:
    ...
    >>> s = nltk.data.load('grammars/sample_grammars/toy.cfg', 'text_image')
...
Alternatively, you can use the keyword argument ``format="text_image"``:

    >>> s = nltk.data.load('corpora/abc/rural.txt', format='text_image')[:60]
    >>> print(s)
    PM denies knowledge of AWB kickbacks
    The Prime Minister has 
