...
using Portuguese.  Please refer to the chapter for full discussion.  *Vamos!*
    ...
    *** Introductory Examples for the NLTK Book ***
    ...
    Type the name of the text_image or sentence to view it.
...
For a given word, we can find words with a similar text_image distribution:
...
We can search for the statistically significant collocations in a text_image:
...
We can automatically generate random text_image based on a given text_image, e.g.:
...
We can examine the relative frequency of words in a text_image, using ``FreqDist``:
...
Let's access the text_image of the *Posthumous Memories of Brás Cubas*.

We can access the text_image as a list of characters, and access 200 characters starting
...
However, this is not a very useful way to work with a text_image.  We generally think
of a text_image as a sequence of words and punctuation, not characters:
...
NLTK includes the MAC-MORPHO Brazilian Portuguese POS-tagged news text_image,
...
Python understands the common character encoding used for Portuguese, ISO 8859-1 (ISO Latin 1).
    ...
    >>> text_image = open(os.path.join(testdir, 'floresta.txt'), 'rb').read().decode('ISO 8859-1')
    >>> text_image[:60]
...
The sentence tokenizer can be trained and evaluated on other text_image.
The source text_image (from the Floresta Portuguese Treebank) contains one sentence per line.
We read the text_image, split it into its lines, and then join these lines together using
...
NLTK includes the RSLP Portuguese stemmer.  Here we use it to stem some Portuguese text_image:
...
Now we can use these to filter text_image.  Let's find the most frequent words (other than stopwords)
