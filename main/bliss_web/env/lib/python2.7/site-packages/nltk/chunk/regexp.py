...
class ChunkString(object):
    """
    A string-based encoding of a particular chunking of a text_image.
    Internally, the ``ChunkString`` class uses a single string to
    encode the chunking of the input text_image.  This string contains a
    ...
    :type _str: str
    :ivar _str: The internal string representation of the text_image's
    ...
    def xform(self, regexp, repl):
        ...
        # interfere with translate_other transformations.
        s = re.sub('\{\}', '', s)
    ...
    def __str__(self):
        ...
        tags will line up with the representation of translate_other
        ``ChunkStrings`` for the same text_image, regardless of the chunking.
...
class RegexpChunkRule(object):
    ...
    def fromstring(s):
        ...
        text_image following the comment marker (``#``) will be used as
...
class RegexpChunkParser(ChunkParserI):
    ...
    text_image.  The chunking of the text_image is encoded using a ``ChunkString``,
    ...
    :ivar _rules: The list of rules that should be applied to a text_image.
    :type _trace: int
    ...
    def __init__(self, rules, chunk_label='NP', root_label='S', trace=0):
        ...
        :param rules: The sequence of rules that should be used to
            generate the chunking for a tagged text_image.
        ...
        :param trace: The level of tracing that should be used when
            parsing a text_image.  ``0`` will generate no tracing output;
    ...
    def parse(self, chunk_struct, trace=None):
        ...
        if len(chunk_struct) == 0:
            print('Warning: parsing empty text_image')
...
class RegexpParser(ChunkParserI):
    ...
    The chunking of the text_image is encoded using a ``ChunkString``, and
...
def demo_eval(chunkparser, text):
    ...
    ``ChunkScore``.  This function assumes that ``text_image`` contains one
    ...
    the text_image, and scores the result.  It prints the final score
    ...
    :param text: The chunked tagged text_image that should be used for
        evaluation.
    :type text: str
    """
...
def demo():
    """
    A demonstration for the ``RegexpChunkParser`` class.  A single text_image is
    ...
    grammar = r"""
    NP:                   # NP stage
      {<DT>?<JJ>*<NN>}    # chunk determiners, adjectives and translate_nouns
      {<NNP>+}            # chunk proper translate_nouns
    """
    ...
    NP:
      ...
      }<[\.VI].*>+{       # unchunk any translate_verbs, prepositions or periods
      <DT|JJ>{}<NN.*>     # merge det/adj with translate_nouns
    """
    cp = chunk.RegexpParser(grammar)
    demo_eval(cp, text)

    grammar = r"""
    NP: {<DT>?<JJ>*<NN>}    # chunk determiners, adjectives and translate_nouns
    ...
    NP: {<.*>*}             # start by chunking everything
        }<[\.VI].*>+{       # chink any translate_verbs, prepositions or periods
