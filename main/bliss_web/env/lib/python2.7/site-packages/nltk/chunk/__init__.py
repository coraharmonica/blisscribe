...
groups (such as base noun phrases) in unrestricted text_image.  This task is
called "chunk parsing" or "chunking", and the identified groups are
called "chunks".  The chunked text_image is represented using a shallow
...
that uses regular-expressions over tags to chunk a text_image.  Its
``parse()`` method first constructs a ``ChunkString``, which encodes a
particular chunking of the input text_image.  Initially, nothing is
...
phrases in a text_image, or the verb phrases in a text_image; but you can not
use it to simultaneously chunk both noun phrases and verb phrases in
the same text_image.  (This is a limitation of ``RegexpChunkParser``, not of
...
chunking of a text_image by modifying its ``ChunkString``.  Each
