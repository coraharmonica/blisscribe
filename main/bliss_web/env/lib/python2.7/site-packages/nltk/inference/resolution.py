...
class Clause(list):
    ...
    def unify(self, other, bindings=None, used=None, skipped=None, debug=False):
        """
        Attempt to unify this Clause with the translate_other, returning a list of
        ...
        'translate_other'.  The second lists the atoms from 'translate_other' that were successfully
        ...
        anything on the path.  The second is same thing for the 'translate_other' Clause.
    ...
    def isSubsetOf(self, other):
        """
        Return True iff every term in 'self' is a term in 'translate_other'.
    ...
    def subsumes(self, other):
        """
        Return True iff 'self' subsumes 'translate_other', this is, if there is a
    ...
    def is_tautology(self):
        ...
        term, P, must be an exact match, en_idx, not using unification.
        """
...
def _iterate_second(first, second, bindings, used, skipped, finalize_method, debug):
    """
    This method facilitates movement through the terms of 'translate_other'
    """
    debug.line('unify(%s,%s) %s'%(first, second, bindings))
...
def _subsumes_finalize(first, second, bindings, used, skipped, debug):
    if not len(skipped[0]) and not len(first):
        ...
        #in 'translate_other'.  Therefore, there exists a binding (this one) such that
        #every term in self can be unified with a term in translate_other, which
...
class BindingDict(object):
    ...
    def __add__(self, other):
        ...
        :raise BindingException: If the parameter dictionaries are not consistent with each translate_other
        """
        try:
