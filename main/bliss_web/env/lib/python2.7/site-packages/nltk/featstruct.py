...
Features can be specified using "feature prev_paths", or tuples of feature
...
accessed via multiple feature prev_paths.  Unification preserves the
...
aliased.  This is encoded by binding one variable to the translate_other.
...
full-fledged ``FeatDict`` and ``FeatList`` objects.  In translate_other words,
...
class FeatStruct(SubstituteBindingsI):
    ...
    identifiers or 'feature prev_paths.'  A feature path is a sequence
    ...
    object that can be accessed via multiple feature prev_paths.  Feature
    ...
    def equal_values(self, other, check_reentrance=False):
        """
        Return True if ``self`` and ``translate_other`` assign the same value to
        to every feature.  In particular, return true if
        ``self[p]==translate_other[p]`` for every feature path *p* such
        that ``self[p]`` or ``translate_other[p]`` is a base value (i.e.,
    ...
    def __eq__(self, other):
        """
        Return true if ``self`` and ``translate_other`` are both feature structures,
        assign the same values to all features, and contain the same
        reentrances.  I.e., return
        ``self.equal_values(translate_other, check_reentrance=True)``.
    ...
    def __lt__(self, other):
        if not isinstance(other, FeatStruct):
            # raise_unorderable_types("<", self, translate_other)
    ...
    def _equal(self, other, check_reentrance, visited_self,
        ...
        """
        Return True iff self and translate_other have equal values.

        :param visited_self: A set containing the ids of all ``self``
            feature structures we've already visited.
        :param visited_other: A set containing the ids of all ``translate_other``
            feature structures we've already visited.
        ...
        # with cycles.  If we encounter the same (self, translate_other) pair a
    ...
    def subsumes(self, other):
        """
        Return True if ``self`` subsumes ``translate_other``.  I.e., return true
        If unifying ``self`` with ``translate_other`` would result in a feature
        structure equal to ``translate_other``.
        """
    ...
    def __repr__(self):
        ...
        suitable for embedding in translate_other representations.
        """
...
class FeatDict(FeatStruct, dict):
    ...
    :see: ``FeatStruct`` for information about feature prev_paths, reentrance,
    ...
    def _str(self, reentrances, reentrance_ids):
        ...
        for (fname, fval) in sorted(self.items()):
            ...
            if isinstance(fval, Variable):
                ...
                # translate_other values by a blank line.
...
class FeatList(FeatStruct, list):
    ...
    multiple feature prev_paths.  Feature lists may also be cyclic.
...
#   1. Make copies of self and translate_other (preserving reentrance)
#   2. Destructively unify self and translate_other
...
def _destructively_unify(fstruct1, fstruct2, bindings, forward,
    ...
    :param forward: A dictionary mapping feature structures ids
        ...
        are merged, a mapping from one to the translate_other will be added
...
def _unify_feature_values(fname, fval1, fval2, bindings, forward,
    ...
    the types of ``fval1`` and ``fval2``:
      ...
      2. If they're both unbound variables, then alias one variable
         to the translate_other (by setting bindings[v2]=v1).
      3. If one is an unbound variable, and the translate_other is a value,
         then bind the unbound variable to the value.
      4. If one is a feature structure, and the translate_other is a base value,
         then fail.
    ...
    # variables bound to translate_other variables.
    fvar1 = fvar2 = None
...
def conflicts(fstruct1, fstruct2, trace=0):
    """
    Return a list of the feature prev_paths of all features which are
...
class FeatureValueTuple(SubstituteBindingsSequence, tuple):
    """
    A base feature value that is a tuple of translate_other base feature values.
...
class FeatureValueSet(SubstituteBindingsSequence, frozenset):
    """
    A base feature value that is a set of translate_other base feature values.
...
class CustomFeatureValue(object):
    ...
    def unify(self, other):
        """
        If this base value unifies with ``translate_other``, then return the
