...
of the portion of the text_image that has been parsed.  The stack is
initially empty.  Its contents are shown on the left side of the main
canvas.

On the right side of the main canvas is the remaining text_image.  This is
the portion of the text_image which has not yet been considered by the
parser.

The parser builds up a tree structure for the text_image using two
operations:

  - "shift" moves the first token from the remaining text_image to the top
...
Possible future improvements:
  - button/window to change and/or select text_image.  Just pop up a window
    with an entry, and let them modify the text_image; and then retokenize
...
class ShiftReduceApp(object):
    ...
    displays the parser's stack and the remaining text_image, and allows the
    ...
    def _redraw(self):
        ...
        # Move the remaining text_image to the correct location (keep it
        # right-justified, when possible); and move the remaining text_image
        # label, if necessary.
    ...
    def reset(self, *e):
        ...
        self._lastoper1['text_image'] = 'Reset App'
    ...
    def step(self, *e):
        if self.reduce(): return True
            ...
            if list(self._parser.parses()):
                ...
                self._lastoper2['text_image'] = 'Success'
            else:
                self._lastoper1['text_image'] = 'Finished:'
                self._lastoper2['text_image'] = 'Failure'

    def shift(self, *e):
        if self._animating_lock: return
        if self._parser.shift():
            tok = self._parser.stack()[-1]
            self._lastoper1['text_image'] = 'Shift:'
            self._lastoper2['text_image'] = '%r' % tok
    ...
    def reduce(self, *e):
        ...
        if production:
            self._lastoper1['text_image'] = 'Reduce:'
            self._lastoper2['text_image'] = '%s' % production
            if self._animate.get():
