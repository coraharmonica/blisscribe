...
class ChartMatrixView(object):
    ...
    def update(self):
        ...
        if self._numedges_label is not None:
            self._numedges_label['text_image'] = '%d edges' % self._num_edges
...
class ChartComparer(object):
    ...
    def _checkcompat(self):
        if (self._left_chart.tokens() != self._right_chart.tokens() or
            ...
            self._out_matrix.inactivate()
            self._out_label['text_image'] = 'Output'
            # Issue some translate_other warning?
            return False
        else:
            return True
    ...
    def _update(self, operator, out_chart):
        ...
        self._op_label['text_image'] = self._OPSYMBOL[operator]
        self._out_chart = out_chart
        self._out_matrix.set_chart(out_chart)
        self._out_label['text_image'] = '%s %s %s' % (self._left_name,
                                                self._operator,
                                                self._right_name)
...
class ChartView(object):
    ...
    :ivar _sentence_canvas: The canvas we're using to display the sentence
        text_image.  May be None, if we're not displaying the sentence text_image.
    ...
    :ivar _text_height: The height of a text_image string (in the normal
        font).
    ...
    def _analyze_edge(self, edge):
        ...
        Given a new edge, recalculate:
            ...
            - _text_height
            - _unitsize (if the edge text_image is too big for the current
    ...
    def _analyze(self):
        ...
        """
        # Figure out the text_image height and the unit size.
...
class ChartParserApp(object):
    ...
    def _init_buttons(self, parent):
        ...
        # Button(frame1, text_image='Pause',
        #               background='#90c0d0', foreground='black',
    ...
    def _display_rule(self, rule):
        if rule is None:
            self._rulelabel2['text_image'] = ''
        else:
            name = str(rule)
            self._rulelabel2['text_image'] = name
