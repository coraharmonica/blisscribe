...
class RegexpChunkApp(object):
    ...
    See ``HELP`` for instructional text_image.
    """
    ...
    #: which is used in the help text_image.  (This should probably live with
    ...
    #:   - The text_image contents for the help page.  You can use expressions
    #:     like <red>...</red> to colorize the text_image; see ``HELP_AUTOTAG``
...
#                 min_precision = 0
    ...
    def _eval_demon(self):
        ...
        if self._eval_index >= self._devset_size.get():
            ...
            self.status['text_image'] = ('Evaluating on Development Set (%d%%)' %
                                   progress)
            self._eval_demon_running = True
    ...
    def show_trace(self, *e):
        ...
        self.devsetlabel['text_image']='Development Set (%d/%d)' % (
    ...
    def _view_history(self, index):
        ...
        if self._history_index < len(self._history)-1:
            self.grammarlabel['text_image'] = 'Grammar %s/%s:' % (
                self._history_index+1, len(self._history))
        else:
            self.grammarlabel['text_image'] = 'Grammar:'
