...
use dynamic programming to efficiently parse a text_image.  A chart
parser derives parse trees for a text_image by iteratively adding "edges"
to a "chart."  Each edge represents a hypothesis about the tree
structure for a subsequence of the text_image.  The chart is a
"blackboard" for composing and combining these hypotheses.

When a chart parser begins parsing a text_image, it creates a new (empty)
chart, spanning the text_image.  It then incrementally adds new edges to the
...
class EdgeI(object):
    ...
    - A ``TreeEdge`` records which trees have been found to
      be (partially) consistent with the text_image.
    - A ``LeafEdge`` records the tokens occurring in the text_image.
...
class TreeEdge(EdgeI):
    ...
    def __init__(self, span, lhs, rhs, dot=0):
        ...
        :param dot: The position of the new edge's dot.  This position
            ...
            is consistent with the text_image.  In particular, if
...
class SingleEdgeFundamentalRule(FundamentalRule):
    ...
    if the translate_other edge is already in the chart.
...
class SteppingChartParser(ChartParser):
    ...
    The ``initialize`` method is used to start parsing a text_image.  ``step``
