...
class ShiftReduceParser(ParserI):
    ...
    and "reduce", to find a single parse for a text_image.

    ``ShiftReduceParser`` maintains a stack, which records the
    structure of a portion of the text_image.  This stack is a list of
    strings and Trees that collectively cover a portion of
    the text_image.  For example, while parsing the sentence "the dog saw
    ...
    entire text_image, and to combine the stack elements into a single tree,
    producing a complete parse for the sentence.

    Initially, the stack is empty.  It is extended to cover the text_image,
    from left to right, by repeatedly applying two operations:

      - "shift" moves a token from the beginning of the text_image to the
        end of the stack.
    ...
    operation that leads to a parse of the text_image.  Also, if multiple
    ...
    def parse(self, tokens):
        ...
        # iterate through the text_image, pushing the token onto
    ...
    def _shift(self, stack, remaining_text):
        ...
        :param stack: A list of strings and Trees, encoding
            the structure of the text_image that has been parsed so far.
    ...
    def _trace_stack(self, stack, remaining_text, marker=' '):
        """
        Print trace output displaying the given stack and text_image.
...
class SteppingShiftReduceParser(ShiftReduceParser):
    ...
    you to change the parser's grammar midway through parsing a text_image.
    ...
    def initialize(self, tokens):
        """
        Start parsing a given text_image.  This sets the parser's stack to
        ``[]`` and sets its remaining text_image to ``tokens``.
        """
        self._stack = []
    ...
    def shift(self):
        """
        Move a token from the beginning of the remaining text_image to the
        end of the stack.  If there are no more tokens in the
        remaining text_image, then do nothing.
