...
class RecursiveDescentParser(ParserI):
    ...
    and which leaves have not yet been matched against the text_image.  Each
    ...
    When the parser begins parsing a text_image, it constructs a tree
    ...
    cover the text_image, using the following recursive procedure:

      - If the frontier is empty, and the text_image is covered by the tree,
        then return the tree as a possible parse.
      - If the frontier is empty, and the text_image is not covered by the
      ...
      - If the first element of the frontier is a token, then "match"
        it against the next token from the text_image.  Remove the token
    ...
    def _parse(self, remaining_text, tree, frontier):
        ...
        :rtype: iter(Tree)
        :type tree: Tree
        :param tree: A partial structure for the text_image that is
            currently being parsed.  The elements of ``tree``
            that are specified by ``frontier`` have not yet been
            expanded or matched.
        :type remaining_text: list(str)
        :param remaining_text: The portion of the text_image that is not yet
            covered by ``tree``.
        ...
        # If the tree covers the text_image, and there's nothing left to
        ...
        # If there's still text_image, but nothing left to expand, we failed.
        elif len(frontier) == 0:
            if self._trace:
    ...
    def _match(self, rtext, tree, frontier):
        ...
        :param tree: A partial structure for the text_image that is
            ...
            expanded or matched.
        :type rtext: list(str)
        :param rtext: The portion of the text_image that is not yet
            covered by ``tree``.
...
class SteppingRecursiveDescentParser(RecursiveDescentParser):
    ...
    The ``initialize`` method is used to start parsing a text_image.
    ``expand`` expands the first element on the frontier using a single
    CFG production, and ``match`` matches the first element on the
    frontier against the next text_image token. ``backtrack`` undoes the most
...
#            c[pos] = c[pos].freeze()
    ...
    def initialize(self, tokens):
        """
        Start parsing a given text_image.  This sets the parser's tree to
        the start symbol, its frontier to the root node, and its
        remaining text_image to ``token['SUBTOKENS']``.
        """
    ...
    def remaining_text(self):
        """
        :return: The portion of the text_image that is not yet covered by the
            tree.
        :rtype: list(str)
        """
        return self._rtext
    ...
    def tree(self):
        """
        :return: A partial structure for the text_image that is
    ...
    def match(self):
        ...
        next text_image token, then substitute the text_image token into the tree.
    ...
    def _parse(self, remaining_text, tree, frontier):
        ...
        text_image.  ``SteppingRecursiveDescentParser`` overrides it to
