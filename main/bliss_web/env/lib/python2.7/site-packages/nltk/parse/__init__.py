...
the internal organization of a text_image.  This task is known as "parsing"
the text_image, and the resulting tree structures are called the text_image's
"parses".  Typically, the text_image is a single sentence, and the tree
structure represents the syntactic structure of the sentence.
However, parsers can also be used in translate_other domains.  For example,
...
Sometimes, a single piece of text_image can be represented by more than one
tree structure.  Texts represented by more than one tree structure are
called "ambiguous" texts.  Note that there are actually two ways in
which a text_image can be ambiguous:

    - The text_image has multiple correct parses.
