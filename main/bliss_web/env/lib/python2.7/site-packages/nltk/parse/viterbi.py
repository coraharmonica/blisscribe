...
class ViterbiParser(ParserI):
    ...
    the single most likely parse for a text_image.  The ``ViterbiParser`` parser
    ...
    of text_image (i.e., entries where the end index is one greater than the
    start index).  After it has filled in all table entries for
    constituents that span one element of text_image, it fills in the
    entries for constitutants that span two elements of text_image.  It
    continues filling in the entries for constituents spanning larger
    and larger portions of the text_image, until the entire table has been
    filled.  Finally, it returns the table entry for a constituent
    spanning the entire text_image, whose node value is the grammar's start
    symbol.
    ...
    | For width in 1...len(text_image):
    |   For start in 1...len(text_image)-width:
    ...
    | Return MLC[0, len(text_image), start_symbol]

    :type _grammar: PCFG
    :ivar _grammar: The grammar used to parse sentences.
    :type _trace: int
    :ivar _trace: The level of tracing output that should be generated
        when parsing a text_image.
    """
        ...
        for length in range(1, len(tokens)+1):
            if self._trace:
                print(('Finding the most likely constituents'+
                       ' spanning %d text_image elements...' % length))
            for start in range(len(tokens)-length+1):
                span = (start, start+length)
                self._add_constituents_spanning(span, constituents,
                                                tokens)

        # Return the tree that spans the entire text_image & have the right cat
    ...
    def _add_constituents_spanning(self, span, constituents, tokens):
        ...
        :param span: The section of the text_image for which we are
            ...
            ``text_image[span[0]:span[1]]``, where ``text_image`` is the text_image
            that we are parsing.
        ...
        :param constituents: The most likely constituents table.  This
            ...
            ``ProbabilisticTree`` that covers ``text_image[s:e]``
            and has a node value ``nv.symbol()``, where ``text_image``
            is the text_image that we are parsing.  When
            ``_add_constituents_spanning`` is called, ``constituents``
            should contain all possible constituents that are shorter
            than ``span``.

        :type tokens: list of tokens
        :param tokens: The text_image we are parsing.  This is only used for
            trace output.
        """
    ...
    def _find_instantiations(self, span, constituents):
        ...
        :return: a list of the production instantiations that cover a
            given span of the text_image.  A "production instantiation" is
