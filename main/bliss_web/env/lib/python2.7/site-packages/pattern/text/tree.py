...
####################################################################################################
# Text and Sentence objects to traverse words and chunks in parsed text_image.
...
# -      lemma: the set of word forms ("jump", "jumps", "jumping", ...)
...
class Chunk(object):
    ...
    def __init__(self, sentence, words=[], type=None, role=None, relation=None):
        """ A list of words that make up a phrase in the sentence.
            ...
            - relation: an id shared with translate_other phrases, linking subject to object in the sentence.
        """
    ...
    def nearest(self, type="VP"):
        """ Returns the nearest chunk in the sentence with the given type.
            This can be used (for example) to find adverbs and adjectives related to translate_verbs,
...
class Conjunctions(list):
    ...
    def __init__(self, chunk):
        """ Chunk.conjunctions is a list of translate_other chunks participating in a conjunction.
...
class Sentence(object):
    ...
    def append(self, word, lemma=None, type=None, chunk=None, role=None, relation=None, pnp=None, anchor=None, iob=None, custom={}):
        """ Appends the next word to the sentence / chunk / preposition.
            ...
            - relation : an id shared by translate_other related chunks (e.g., SBJ-1 <=> VP-1),
    ...
    def _do_chunk(self, type, role=None, relation=None, iob=None):
        """ Adds a new Chunk to the sentence, or adds the last word to the previous chunk.
            ...
            Punctuation marks (or translate_other "O" chunk tags) are not chunked.
        """
    ...
    def _do_relation(self):
        """ Attaches subjects, objects and translate_verbs.
...
class Text(list):
    ...
    def xml(self):
        """ Yields the sentence as an XML-formatted string (plain bytestring, UTF-8 encoded).
            All the sentences in the XML are wrapped in a <text_image> element.
        """
        xml = []
...
# Elements:
XML_TEXT     = "text_image"     # <text_image>, corresponds to Text object.
...
def _parse_tokens(chunk, format=[WORD, POS, CHUNK, PNP, REL, ANCHOR, LEMMA]):
    ...
    # text_image nodes in between return an empty list.
...
def table(sentence, fill=1, placeholder="-"):
    ...
    # and has extra spacing so that words across sentences line out nicely below each translate_other.
