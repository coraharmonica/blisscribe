...
def removeDuplicates(sequence):
    ...
    >>> remove_duplicates("this is a test")
    ['t', 'h', 'i', 's', ' ', 'a', 'e']
    >>> remove_duplicates(map(lambda tuple:apply(meet, tuple), product(N['story'].getSenses(), N['joke'].getSenses())))
...
def getIndex(form, pos='noun'):
    ...
    index file corresponding to _pos_.  get_word() is called on each
