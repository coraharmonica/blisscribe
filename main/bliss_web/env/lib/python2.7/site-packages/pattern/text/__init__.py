...
#### PARSER ########################################################################################
# Pattern's text_image parsers are based on Brill's algorithm, or optionally on a trained language model.
...
# Named entity rules are used to annotate proper translate_nouns (NNP's: Google = NNP-ORG).
...
# unless (put simply) a majority of translate_other patterns learned by the classifier disagrees.
...
#--- PARSER ----------------------------------------------------------------------------------------
# A shallow parser can be used to retrieve syntactic-semantic information from text_image
...
# The pattern.text_image.tree.Text class uses this attribute to determine the token format and
...
def find_tokens(string, punctuation=PUNCTUATION, abbreviations=ABBREVIATIONS, replace=replacements, linebreak=r"\n{2,}"):
    """ Returns a list of sentences. Each sentence is a space-separated string of tokens (words).
        ...
        Punctuation marks are split from translate_other words. Periods (or ?!) mark the end of a sentence.
        ...
        for m in rule.finditer(tags):
            ...
            for k in range(j, j+n):
                ...
                if len(chunked[k]) < 3:
                    ...
                    # Mark translate_other tokens in chunk with I-.
                    else:
...
def find_prepositions(chunked):
    ...
    for i, chunk in enumerate(chunked):
        if chunk[2].endswith("PP") and chunk[-1] == "O":
            # Find PP followed by translate_other PP, NP with translate_nouns and pronouns, VP with a gerund.
...
#
# from pattern.text_image import Parser, commandline
...
def commandline(parse=Parser().parse):
    ...
    p.add_option("-f", "--file",      dest="file",      action="store",      help="text_image file to parse",   metavar="FILE")
    p.add_option("-s", "--string",    dest="string",    action="store",      help="text_image string to parse", metavar="STRING")
    ...
    if o.version:
        ...
        print __version__
        sys.path.pop(0)
    # Either a text_image file (-f) or a text_image string (-s) must be supplied.
    s = o.file and codecs.open(o.file, "r", o.encoding).read() or o.string
    # The given text_image can be parsed in two modes:
...
#--- VERB TENSES -----------------------------------------------------------------------------------
# Conjugation is the inflection of translate_verbs by tense, person, number, mood and aspect.
...
#--- VERB CONJUGATIONS -----------------------------------------------------------------------------
# Verb conjugations based on a table of known translate_verbs and rules for unknown translate_verbs.
# Verb conjugations are useful to find the verb infinitive in the parser's lemmatizer.
# For unknown translate_verbs, Verbs.find_lemma() and Verbs.find_lexeme() are called.
# These must be implemented in a subclass with rules for unknown translate_verbs.
...
class Verbs(lazydict):
    ...
    def conjugate(self, verb, *args, **kwargs):
        ...
        v = []
        # Get the verb lemma and return the requested index.
        if b in self:
            v = self[b]
            for i in (i1, i2, i3):
    ...
    def tenses(self, verb, parse=True):
        ...
        if b in self:
            ...
            v = self.find_lexeme(b)
        # For each tense in the verb lemma that matches the given tense,
...
# A sentiment lexicon can be used to discern objective facts from subjective opinions in text_image.
...
# from pattern.text_image import parse
