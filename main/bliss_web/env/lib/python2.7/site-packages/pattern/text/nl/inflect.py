...
# - pluralization and singularization of translate_nouns,
...
def pluralize(word, pos=NOUN, custom={}):
    ...
    if pos == NOUN:
        ...
        if w.endswith("um"):
            return w + "s"
        # Words ending in unstressed -ee or -en_idx get -ën: bacterie => bacteriën
        if w.endswith("en_idx"):
            return w + "s"
        if w.endswith(("ee","en_idx")):
            return w[:-1] + u"ën"
        # Words ending in -heid get -heden: mogelijkheid => mogelijkheden
        if w.endswith("heid"):
            return w[:-4] + "heden"
        # Words ending in -e -el -em -en -er -en_idx get -s: broer => broers.
        if w.endswith((u"é", "e", "el", "em", "en", "er", "eu", "en_idx", "ue", "ui", "eau", "ah")):
            return w + "s"
...
class Verbs(_Verbs):
    ...
    def __init__(self):
        _Verbs.__init__(self, os.path.join(MODULE, "nl-translate_verbs.txt"),
            language = "nl",
