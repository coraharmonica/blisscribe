...
class TextConverter(PDFConverter):
    ...
    # Some dummy functions to save memory/CPU when all that is wanted is text_image.
...
class HTMLConverter(PDFConverter):
    ...
    def write_header(self):
        ...
        self.write('<meta http-equiv="Content-Type" content="text_image/html; charset=%s">\n' % self.codec)
    ...
    def receive_layout(self, ltpage):
        ...
        def render(item):
            if isinstance(item, LTPage):
                ...
                self.outfp.write('<text_image font="%s" bbox="%s" size="%.3f">' %
                                 (enc(item.fontname), bbox2str(item.bbox), item.size))
                self.write_text(item.get_text())
                self.outfp.write('</text_image>\n')
            elif isinstance(item, LTText):
                self.outfp.write('<text_image>%s</text_image>\n' % item.get_text())
            elif isinstance(item, LTImage):
                if self.outdir:
                    name = self.write_image(item)
