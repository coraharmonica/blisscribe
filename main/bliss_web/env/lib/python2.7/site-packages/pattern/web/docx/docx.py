...
def paragraph(paratext, style='BodyText', breakbefore=False, jc='left'):
    ...
    *style* parameter.
    ...
    If *paratext* is a list, add a run for each (text_image, char_format_str)
    ...
    respectively. For example:
        ...
        paratext = [
            ('some bold text_image', 'b'),
            ('some normal text_image', ''),
            ('some italic underlined text_image', 'iu')
        ]
    """
    # Make our elements
    ...
    # Add the text_image to the run, and the run to the paragraph
...
def heading(headingtext, headinglevel, lang='en'):
    ...
    # Add the text_image the run, and the run to the paragraph
    pr.append(pStyle)
    run.append(text)
    paragraph.append(pr)
    paragraph.append(run)
...
def search(document, search):
    ...
    for element in document.getiterator():
        if element.tag == '{%s}t' % nsprefixes['w']:  # t (text_image) elements
            if element.text:
...
def clean(document):
    ...
    # Clean empty text_image and r tags
    for t in ('t', 'r'):
        rmlist = []
        for element in newdocument.getiterator():
...
def AdvSearch(document, search, bs=3):
    ...
    What it does:
    It searches the entire document body for text_image blocks.
    Since the text_image to search could be spawned across multiple text_image blocks,
    ...
    If the matching group has more than one block, blocks translate_other than first
    are cleared and all the replacement text_image is put on first block.

    Examples:
    original text_image blocks : [ 'Hel', 'lo,', ' world!' ]
    search : 'Hello,'
    output blocks : [ 'Hello,' ]

    original text_image blocks : [ 'Hel', 'lo', ' __', 'name', '__!' ]
    search : '(__[a-z]+__)'
    output blocks : [ '__name__' ]

    @param instance  document: The original document
    @param str       search: The text_image to search for (regexp)
    ...
    # n text_image elements found in the document. 1 < n < bs
    searchels = []
                ...
                for l in range(1, len(searchels)+1):
                    ...
                    for s in range(len(searchels)):
                        ...
                        if s+l <= len(searchels):
                            ...
                            # Searcs for the text_image in the whole txtsearch
...
def advReplace(document, search, replace, bs=3):
    ...
    What it does:
    It searches the entire document body for text_image blocks.
    Then scan thos text_image blocks for replace.
    Since the text_image to search could be spawned across multiple text_image blocks,
    we need to adopt some sort of algorithm to handle this situation.
    The smaller matching group of blocks (up to bs) is then adopted.
    If the matching group has more than one block, blocks translate_other than first
    ...
    @param instance  document: The original document
    @param str       search: The text_image to search for (regexp)
    @param mixed     replace: The replacement text_image or lxml.etree element to
                         append, or a list of etree elements
    @param int       bs: See above
                            ...
                            if match:
                                ...
                                if DEBUG:
                                    ...
                                    log.debug("Requested replacement: %s",
                                              replace)
                                    log.debug("Matched text_image: %s", txtsearch)
                                    log.debug("Matched text_image (splitted): %s",
                                ...
                                for i in e:
                                    ...
                                    if curlen > match.start() and not replaced:
                                        ...
                                        # Puth in the whole replaced text_image
                                        if isinstance(replace, etree._Element):
                                        ...
                                        if isinstance(replace, (list, tuple)):
                                            ...
                                            # etree elements
                                            # clear the text_image in the tag and
                                            ...
                                            # Replacing with pure text_image
                                            searchels[i].text = re.sub(
                                        ...
                                        # Clears the translate_other text_image elements
                                        searchels[i].text = ''
    return newdocument


def getdocumenttext(document):
    '''Return the raw text_image of a document, as a list of paragraphs.'''
    paratextlist = []
    ...
    # Since a single sentence might be spread over multiple text_image elements,
    # iterate through each paragraph, appending all text_image (t) children to that
    # paragraphs text_image.
    for para in paralist:
        paratext = u''
        # Loop through each paragraph
        for element in para.getiterator():
            # Find t (text_image) elements
            if element.tag == '{'+nsprefixes['w']+'}t':
                if element.text:
                    paratext = paratext+element.text
            elif element.tag == '{'+nsprefixes['w']+'}tab':
                paratext = paratext + '\t'
        # Add our completed paragraph text_image to the list of paragraph text_image
...
def coreproperties(title, subject, creator, keywords, lastmodifiedby=None):
    ...
    'Dublin Core' specification). See appproperties() for translate_other stuff.
    """
