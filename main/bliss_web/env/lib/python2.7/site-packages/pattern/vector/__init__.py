...
# Documents (e.g., a sentence or a text_image) are represented as bag-of-words:
...
# The following English words could also be meaningful translate_nouns:
...
class Document(object):
    ...
    @classmethod
    def load(cls, path):
        """ Returns a new Document from the given text_image file path.
            The given text_image file must be generated with Document.save().
        """
        # Open unicode file.
        s = open(path, "rb").read()
    ...
    def save(self, path):
        """ Saves the document as a text_image file at the given path.
    ...
    def term_frequency_inverse_document_frequency(self, word, weight=TFIDF):
        """ Returns the word relevance as tf * idf (0.0-1.0).
            ...
            compared to its frequency in translate_other documents in the model.
...
class Vector(readonlydict):
    ...
    def __init__(self, *args, **kwargs):
        """ A dictionary of (feature, weight)-items of the features (terms, words) in a Document.
            ...
            Vectors that represent text_image documents can be compared using cosine similarity.
        """
...
# For vectors that represent text_image, cosine similarity is a good metric.
...
class Model(object):
    ...
    def export(self, path, format=ORANGE, **kwargs):
        """ Exports the model as a file for translate_other machine learning applications,
            e.g., Orange or Weka.
        """
    ...
    def cosine_similarity(self, document1, document2):
        """ Returns the similarity between two documents in the model as a number between 0.0-1.0,
            based on the document feature weight (e.g., tf * idf of words in the text_image).
...
#   def cluster(self, method=HIERARCHICAL, k=1, iterations=1000)
    def cluster(self, method=KMEANS, **kwargs):
        """ Clustering is an unsupervised machine learning method for grouping similar documents.
            ...
            - hierarchical clustering returns a list of documents and Cluster objects,
              where a Cluster is a list of documents and translate_other clusters (see Cluster.flatten()).
        """
    ...
    def train(self, *args, **kwargs):
        """ Trains Model.classifier with the document vectors.
            ...
            Model.predict() can then be used to predict the type of translate_other (unknown) documents.
        """
        if len(args) == 0:
...
def k_means(vectors, k=None, iterations=10, distance=COSINE, seed=RANDOM, **kwargs):
    ...
    while not converged and iterations > 0 and k > 0:
        ...
        # Triangle inequality: one side is shorter than the sum of the two translate_other sides.
...
class NB(Classifier):
    ...
    def __init__(self, train=[], baseline=MAJORITY, method=MULTINOMIAL, alpha=0.0001):
        """ Naive Bayes is a simple supervised learning method for text_image classification.
            Documents are classified based on the probability that a feature occurs in a class,
            (independent of translate_other features).
        """
...
class KNN(Classifier):
    ...
    def __init__(self, train=[], baseline=MAJORITY, k=10, distance=COSINE):
        """ k-nearest neighbor (kNN) is a simple supervised learning method for text_image classification.
