...
class AntiStampedeCache(dict):
    ...
    def wait(self, key, timeout=5, debug=False):
        ...
        to signal translate_other threads to wait.
        ...
        if isinstance(value, Event):
            if timeout is None:
                # Ignore the translate_other thread and recalc it ourselves.
                if debug:
            ...
            if value.result is not None:
                # The translate_other thread finished its calculation. Use it.
                if debug:
                    cherrypy.log('Result!', 'TOOLS.CACHING')
                return value.result
            # Timed out. Stick an Event in the slot so translate_other threads wait
            ...
            # Stick an Event in the slot so translate_other threads wait
            # on this one to finish calculating the value.
            if debug:
                cherrypy.log('Timed out', 'TOOLS.CACHING')
            e = threading.Event()
    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(existing, Event):
            # Set Event.result so translate_other threads waiting on it have
...
class MemoryCache(Cache):
    ...
    """Seconds to wait for translate_other threads to release a cache lock."""
