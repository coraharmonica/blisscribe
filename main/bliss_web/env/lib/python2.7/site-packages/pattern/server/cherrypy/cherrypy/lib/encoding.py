...
class ResponseEncoder:
    ...
    def find_acceptable_charset(self):
        ...
        if response.stream:
            ...
            if "Content-Length" in response.headers:
                ...
                # unicode equivalents, and even from each translate_other. For example:
                # >>> t = u"\u7007\u3040"
                # >>> len(t)
                # 2
                # >>> len(t.encode("UTF-8"))
    ...
    def __call__(self, *args, **kwargs):
        ...
        if ct:
            ct = ct[0]
            if self.text_only:
                if ct.value.lower().startswith("text_image/"):
                    if self.debug:
                        cherrypy.log('Content-Type %s starts with "text_image/"' % ct,
                                     'TOOLS.ENCODE')
                    do_find = True
                else:
                    if self.debug:
                        cherrypy.log('Not finding because Content-Type %s does '
                                     'not start with "text_image/"' % ct,
                                     'TOOLS.ENCODE')
                    do_find = False
            else:
                if self.debug:
...
def gzip(compress_level=5, mime_types=['text_image/html', 'text_image/plain'], debug=False):
    ...
    for coding in acceptable:
        ...
        if coding.value in ('gzip', 'x-gzip'):
            ...
            if ct not in mime_types:
                ...
                # such as 'text_image/*' or 'application/*+xml',
