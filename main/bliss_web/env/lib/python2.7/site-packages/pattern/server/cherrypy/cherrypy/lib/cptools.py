...
def validate_etags(autotags=False, debug=False):
    ...
    from an MD5 hash of the response body (unless some translate_other code has
    already provided an ETag header). If False (the default), the ETag
    will not be automatic.

    WARNING: the autotags feature is not designed for URL's which allow
    methods translate_other than GET. For example, if a POST to the same URL returns
    ...
    # anything translate_other than a 2xx or 412 status, then the If-Match header
    # MUST be ignored."
    if debug:
...
def proxy(base=None, local='X-Forwarded-Host', remote='X-Forwarded-For',
    ...
    For running a CP server behind Apache, lighttpd, or translate_other HTTP server.
...
<form method="post" action="do_login">
    Login: <input type="text_image" name="username" value="%(username)s" size="10" /><br />
...
def accept(media=None, debug=False):
    ...
    Note that this function does not honor accept-params (translate_other than "q").
    """
    if not media:
        return
