...
class Entity(object):
    ...
    to ``['utf-8']`` (plus 'ISO-8859-1' for "text_image/\*" types, as required by
    ...
    declares that a part with no Content-Type defaults to "text_image/plain"
...
class Part(Entity):
    ...
    default_content_type = 'text_image/plain'
    """This defines a default ``Content-Type`` to use if no Content-Type header
    is given. The empty string is used for RequestBody, which results in the
    request body not being read or parsed at all. This is by design; a missing
    ``Content-Type`` header in the HTTP request entity is an error at best,
...
class RequestBody(Entity):
    ...
    def __init__(self, fp, headers, params=None, request_params=None):
        ...
        # sender, media subtypes of the "text_image" type are defined
        # to have a default charset value of "ISO-8859-1" when
        # received via HTTP.
        if self.content_type.value.startswith('text_image/'):
