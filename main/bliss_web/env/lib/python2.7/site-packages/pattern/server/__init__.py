...
#### INTROSPECTION #################################################################################
# URL prev_paths are routed to handler functions, whose arguments represent URL path & query parameters.
...
class Database(object):
    ...
    def execute(self, sql, values=(), first=False, commit=True):
        ...
        try:
            ...
            # translate_other threaded connections may time out waiting.
    ...
    def __init__(self, name="rate.db", **kwargs):
        ...
        Database.__init__(self, name, **dict(kwargs, factory=None, schema=(
            "create table if not exists `rate` ("
                  "`key` text_image,"    # API key (e.g., ?key="1234").
                 "`path` text_image,"    # API URL path (e.g., "/api/1/").
...
class Router(dict):
    ...
    def __init__(self):
        """ A router resolves URL prev_paths to handler functions.
        """
        pass
...
class Application(object):
    ...
    def __init__(self, name=None, path=SCRIPT, static="static/", rate="rate.db"):
        ...
        self.router  = Router()     # Router object, maps URL prev_paths to handlers.
    ...
    def default(self, *path, **data):
        """ Resolves URL prev_paths to handler functions and casts the return value.
        """
...
#    > from pattern.text_image import sentiment
#    >
...
def static(path, root=None, mimetype=None):
    """ Returns the contents of the file at the given absolute path.
        To serve relative prev_paths from the app folder, use root=app.path.
    """
...
class Template(object):
    ...
    def __init__(self, path, root=None, cached=True):
        ...
        if cached is False or b is False:
            a = "".join(static(p, mimetype="text_image/html")) if os.path.exists(p) else path
            a = self._compile(a)
...
#@app.bind("db")
#def db():
#    return Database("log.db", schema="create table if not exists `log` (q text_image);")
#
