...
class _HandleSignalsPlugin(object):
    """Handle signals from translate_other processes based on the configured
...
# The only translate_other way would be to change what is returned from type(request)
...
# import _cpconfig last so it can reference translate_other top-level objects
