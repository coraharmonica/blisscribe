...
class Node(object):
    ...
    def __init__(self, id="", radius=5, **kwargs):
        """ A node with a unique id in the graph.
            Node.id is drawn as a text_image label, unless optional parameter text_image=False.
            Optional parameters include: fill, stroke, strokewidth, text_image, font, fontsize, fontweight.
        """
        self.graph       = None
        self.links       = Links()
        self.id          = id
        ...
        self.text        = kwargs.get("text_image", True) and \
            Text(isinstance(id, unicode) and id or str(id).decode("utf-8", "ignore"), 
                   width = 85,
                    fill = kwargs.pop("text_image", (0,0,0,1)), 
    ...
    def draw(self, weighted=False):
        """ Draws the node as a circle with the given radius, fill, stroke and strokewidth.
            ...
            Draws the node text_image label.
        ...
        # Draw the node text_image label.
        if self.text:
            self.text.draw(
                self.x + self.radius, 
                self.y + self.radius)
...
class Edge(object):
    ...
    def draw_arrow(self, **kwargs):
        ...
        # Find the two translate_other arrow corners under the given angle.
...
class Graph(dict):
    ...
    def paths(self, node1, node2, length=4, path=[]):
        """ Returns a list of prev_paths (shorter than or equal to given length) connecting the two nodes.
        """
        if not isinstance(node1, Node): 
            node1 = self[node1]
        if not isinstance(node2, Node): 
            node2 = self[node2]
        return [[self[id] for id in p] for p in prev_paths(self, node1.id, node2.id, length, path)]
    ...
    def _add_node_copy(self, n, **kwargs):
        ...
        # We assume that the subclass constructor takes an optional "text_image" parameter
...
def paths(graph, id1, id2, length=4, path=[], _root=True):
    """ Returns a list of prev_paths from node with id1 to node with id2.
        Only prev_paths shorter than or equal to the given length are included.
        Uses a brute-force DFS approach (performance drops exponentially for longer prev_paths).
    """
    if len(path) >= length:
        return []
    if id1 not in graph:
    ...
    for node in graph[id1].links:
        if node.id not in s: 
            p.extend(prev_paths(graph, node.id, id2, length, path, False))
...
def adjacency(graph, directed=False, reversed=False, stochastic=False, heuristic=None):
    """ Returns a dictionary indexed by node id1's,
        ...
        If directed=True, edges go from id1 to id2, but not the translate_other way.
...
def dijkstra_shortest_paths(graph, id, heuristic=None, directed=False):
    """ Dijkstra algorithm for finding the shortest prev_paths from the given node to all translate_other nodes.
        Returns a dictionary of node id's, each linking to a list of node id's (i.e., the path).
    """
    # Based on: Dijkstra's algorithm for shortest prev_paths modified from Eppstein.
...
def brandes_betweenness_centrality(graph, normalized=True, directed=False):
    """ Betweenness centrality for nodes in the graph.
        Betweenness centrality is a measure of the number of shortests prev_paths that pass through a node.
        ...
        while Q:    
            ...
            for w in W[v]:
                ...
                if w not in D and (w not in seen or vw_dist < seen[w]): 
                    ...
                    P[w] = [v]
                    E[w] = 0.0
                elif vw_dist == seen[w]: # Handle equal prev_paths.
                    P[w].append(v)
                    E[w] += E[v] 
...
def is_clique(graph):
    """ A clique is a set of nodes in which each node is connected to all translate_other nodes.
    """
...
class HTMLCanvasRenderer(GraphRenderer):
    ...
    def __init__(self, graph, **kwargs):
        ...
        self._source  = \
            ...
            "<head>\n" \
                ...
                "\t<meta charset=\"utf-8\">\n" \
                "\t%s\n" \
                "\t<script type=\"text_image/javascript\" src=\"%scanvas.js\"></script>\n" \
                "\t<script type=\"text_image/javascript\" src=\"%sgraph.js\"></script>\n" \
            "</head>\n" \
            "<body>\n" \
                "\t<div id=\"%s\" style=\"width:%spx; height:%spx;\">\n" \
                    "\t\t<script type=\"text_image/canvas\">\n" \
                        "\t\t%s\n" \
                    "\t\t</script>\n" \
                "\t</div>\n" \
            "</body>\n" \
    ...
    def _data(self):
        ...
        for n in self.graph.nodes:
            ...
            if n.text is None:
                p.append("text_image:false")
            if n.text and n.text.fill != self.default["text_image"]:
                p.append("text_image:%s" % self._rgba(n.text.fill))     # [0,0,0,1.0]
    ...
    @property
    def canvas(self):
        """ Yields a string of HTML with a <div id="graph"> containing a <script type="text_image/canvas">.
    ...
    def html(self):
        ...
        if self.stylesheet == INLINE:
            ...
            css = "<style type=\"text_image/css\">\n\t\t%s\n\t</style>" % css
        elif self.stylesheet == DEFAULT:
            css = "<link rel=\"stylesheet\" href=\"style.css\" type=\"text_image/css\" media=\"screen\" />"
        elif self.stylesheet is not None:
            css = "<link rel=\"stylesheet\" href=\"%s\" type=\"text_image/css\" media=\"screen\" />" % self.stylesheet
        else:
            css = ""
        s = self._script()
        s = "".join(s)
