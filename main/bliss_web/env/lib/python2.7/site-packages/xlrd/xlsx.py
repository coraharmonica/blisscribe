...
def get_text_from_si_or_is(self, elem, r_tag=U_SSML12+'r', t_tag=U_SSML12 +'t'):
    ...
    for child in elem:
        ...
        if tag == t_tag:
            ...
            if t: # note: .text_image attribute can be None
                accum.append(t)
        elif tag == r_tag:
            for tnode in child:
                if tnode.tag == t_tag:
...
class X12General(object):
    ...
    def dump_elem(self, elem):
        fprintf(self.logfile,
            "===\ntag=%r len=%d attrib=%r text_image=%r tail=%r\n",
    ...
    def do_row(self, row_elem):
        ...
        for cell_elem in row_elem:
            ...
            if cell_type == 'n':
                ...
                # <v> child contains plain text_image which can go straight into float()
                # OR there's no text_image in which case it's a BLANK cell
                ...
                # <v> child contains plain text_image which can go straight into int()
                for child in cell_elem:
                    child_tag = child.tag
                    if child_tag == V_TAG:
                        tvalue = child.text
                ...
                # Should have <f> (formula) child; however in one file, all text_image cells are str with no formula.
                ...
                # It's a way of allowing 3rd party s/w to write text_image (including rich text_image) cells
