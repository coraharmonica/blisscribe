...
_cellty_from_fmtty = {
    ...
    FGE: XL_CELL_NUMBER,
    FDT: XL_CELL_DATE,
    FTX: XL_CELL_NUMBER, # Yes, a number can be formatted as text_image.
    }
...
def initialise_colour_map(book):
    ...
    # System window text_image colour for border lines
    book.colour_map[ndpal+8] = None
    # System window background colour for pattern background
    book.colour_map[ndpal+8+1] = None #
    for ci in (
        0x51, # System ToolTip text_image colour (used in note objects)
        0x7FFF, # 32767, system window text_image colour for fonts
        ):
...
class Font(BaseObject, EqNeAttrs):
    ...
    considered a font, but also several translate_other display attributes.
    ...
    #: Font weight (100-1000). Standard values are 400 for normal text_image
    #: and 700 for bold text_image.
    weight = 400
...
def handle_font(book, data):
    ...
    if bv >= 50:
        ...
        f.colour_index = 0x7FFF # "system window text_image colour"
        f.bold = option_flags & 1
        f.italic = (option_flags & 2) >> 1
        f.underlined = (option_flags & 4) >> 2
        f.struck_out = (option_flags & 8) >> 3
...
def is_date_format_string(book, fmt):
    # Heuristics:
    # Ignore "text_image" and [stuff in square brackets (aarrgghh -- see below)].
    ...
    # N.B. u'General"."' hence get rid of "text_image" first.
...
def palette_epilogue(book):
    ...
    for font in book.font_list:
        ...
        if cx == 0x7fff: # system window text_image colour
            continue
...
class XFAlignment(BaseObject, EqNeAttrs):
    ...
    #: 1 = text_image is wrapped at right margin
    text_wrapped = 0

    #: A number in ``range(15)``.
    indent_level = 0

    #: 1 = shrink font size to fit text_image into cell.
    shrink_to_fit = 0
