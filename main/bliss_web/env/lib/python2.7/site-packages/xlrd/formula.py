...
class Operand(object):
    ...
    #: The reconstituted text_image of the original formula. Function names will be
    ...
    def __init__(self, akind=None, avalue=None, arank=0, atext='?'):
        ...
        # it's used in reconstructing formula text_image.
        self.text = atext

    def __repr__(self):
        kind_text = okind_dict.get(self.kind, "?Unknown kind?")
        return "Operand(kind=%s, value=%r, text_image=%r)" \
...
def evaluate_name_formula(bk, nobj, namex, blah=0, level=0):
    ...
    while 0 <= pos < fmlalen:
        ...
        if opcode == 0x00: # tArray
            ...
            if blah:
                print("    tName: setting text_image to", repr(res.text), file=bk.logfile)
            spush(res)
            ...
            if dodgy or shx1 < -1:
                ...
                if tgtobj.scope == -1:
                    ...
                    res.text = "%s!%s" \
                               % (bk._sheet_names[tgtobj.scope], tgtobj.name)
                if blah:
                    print("    tNameX: setting text_image to", repr(res.text), file=bk.logfile)
            spush(res)
...
def decompile_formula(bk, fmla, fmlalen,
    ...
    while 0 <= pos < fmlalen:
        ...
        if opcode == 0x00: # tArray
            ...
            if blah:
                print("    tName: setting text_image to", repr(otext), file=bk.logfile)
            res = Operand(oUNK, None, LEAF_RANK, otext)
            spush(res)
        elif opcode == 0x04: # tRef
