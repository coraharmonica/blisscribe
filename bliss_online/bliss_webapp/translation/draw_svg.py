...
DRAW_SVG:
    ...
    Taken from http://code.activestate.com/recipes/578123-draw-svg-images-in-python-python-recipe-enhanced-v/.
...
class Arc:
    def __init__(self, start, end, color, width, radius):
        self.start = start
        self.end = end
        self.color = color
        self.width = width
    ...
    def strarray(self):
        return ["  <line x1=\"%d\" y1=\"%d\" x2=\"%d\" y2=\"%d\" r=\"%d\" style=\"stroke:%s;stroke-width:%d\"/>\n" % \
                (self.start[0], self.start[1], self.end[0], self.end[1], self.radius, colorstr(self.color), self.width)]
...
def test():
    ...
    scene.add(Arc((200, 200), (200, 300), (0, 0, 0), 1, radius=40))
