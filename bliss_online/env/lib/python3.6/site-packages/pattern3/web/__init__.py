...
def urldecode(query):
    ...
    if query:
        ...
        query = ((decode_string(unquote_plus(encode_string(k))),
                  _format(decode_string(unquote_plus(encode_string(v))))) for k, v in query if k != "")
        return dict(query)
    return {}
...
class URL(object):
    def __init__(self, url_string="", method=GET, query=None, **kwargs):
        ...
        self.__dict__["_string"] = decode_string(url_string)
        self.__dict__["_parts"] = None
        self.__dict__["_headers"] = None
        self.__dict__["_redirect"] = None
        if isinstance(url_string, URL):
            self.__dict__["method"] = url_string.method
            self.query.update(url_string.query)
        if len(query) > 0:
    ...
    def _parse(self):
        ...
        if "@" in parts[DOMAIN]:
            parts[USERNAME], parts[PASSWORD] = (p[1].split("@")[0].split(":") + [""])[:2]
            parts[DOMAIN] = p[1].split("@")[1]
        # Split the port number from the domain.
        if ":" in parts[DOMAIN]:
            parts[DOMAIN], \
            parts[PORT] = parts[DOMAIN].split(":")
            parts[PORT] = parts[PORT].isdigit() and int(parts[PORT]) or parts[PORT]
        # Split the base page from the path.
        if "/" in parts[PATH]:
            parts[PAGE] = p[2].split("/")[-1]
            parts[PATH] = p[2][:len(p[2]) - len(parts[PAGE])].strip("/").split("/")
            parts[PATH] = [v for v in parts[PATH] if v != ""]
        else:
            parts[PAGE] = p[2].strip("/")
            parts[PATH] = []
    ...
    def _set_string(self, v):
        self.__dict__["_string"] = decode_string(v)
        self.__dict__["_parts"] = None
    ...
    def querystring(self):
        ...
        s = dict((encode_string(k), encode_string(v if v is not None else ""))
                 for k, v in s)
        s = urlencode(s)
        return s
    ...
    def __setattr__(self, k, v):
        if k in self.__dict__:
            self.__dict__[k] = decode_string(v)
            return
        if k == "string":
            self._set_string(v)
            return
        if k == "query":
            ...
            return
        if k in self.parts:
            self.__dict__["_parts"][k] = decode_string(v)
            return
        raise AttributeError("'URL' object has no attribute '%s'" % k)
        ...
        try:
            ...
            if authentication is not None:
                request.add_header("Authorization", "Basic %s" %
                                   base64.encodebytes('%s:%s' % authentication))
    ...
    def download(self, timeout=10, cached=True, throttle=0, proxy=None, user_agent=USER_AGENT, referrer=REFERRER,
                 authentication=None, str=False, **kwargs):
        ...
        if cached and id in CACHE:
            ...
            if str is True:
                return CACHE[id]
            if str is False:
                return CACHE.get(id, default=False)
        t = time.time()
    ...
    def __str__(self):
        return encode_string(self.string)
...
def download(url="", method=GET, query=None, timeout=10, cached=True, throttle=0, proxy=None, user_agent=USER_AGENT,
             referrer=REFERRER, authentication=None, str=False):
    """Downloads the content at the given URL (by default it will be cached
    locally).
...
def bind(obj, method, function):
    ...
    setattr(obj, method, methodType(function, obj))
...
def find_email(string, unique=True):
    ...
    """
    string = decode_string(string).replace("\u2024", ".")
    matches = []
...
def encode_url(string):
    return quote_plus(encode_string(string))  # "black/white" => "black%2Fwhite".
...
class Result(dict):
    ...
    def __setitem__(self, k, v):
        super().__setitem__(decode_string(k), self._format(v))

    def setdefault(self, k, v=None):
        return super().setdefault(decode_string(k), self._format(v))

    def update(self, *args, **kwargs):
        super().update([(decode_string(k), self._format(v)) for k, v in list(dict(*args, **kwargs).items())])
...
class Google(SearchEngine):
    ...
    def identify(self, string, **kwargs):
        ...
        data = decode_string(data.get("language")), float(data.get("confidence"))
        return data
...
class Yahoo(SearchEngine):
    ...
    def search(self, query, type=SEARCH, start=1, count=10, sort=RELEVANCY, size=None, cached=True, **kwargs):
        ...
        """
        yahoo_types = {SEARCH: "web", IMAGE: "images", NEWS: "news"}
        if type not in yahoo_types:
            raise SearchEngineTypeError
        else:
            url = YAHOO + yahoo_types[type]
...
class Bing(SearchEngine):
    ...
    def search(self, query, type=SEARCH, start=1, count=10, sort=RELEVANCY, size=None, cached=True, **kwargs):
        ...
        """
        bing_types = {SEARCH: "Web", IMAGE: "Image", NEWS: "News"}
    ...
    def search(self, query, type=SEARCH, start=None, count=None, sort=RELEVANCY, size=None, cached=True, **kwargs):
        ...
        for topic in data.get("RelatedTopics", []):
            for x in topic.get("Topics", [topic]):
                ...
                s1 = decode_string(decode_url(s1.encode("utf-8")))
...
class Twitter(SearchEngine):
    ...
    def search(self, query, type=SEARCH, start=1, count=10, sort=RELEVANCY, size=None, cached=False, **kwargs):
        ...
        if isinstance(start, str):
            result_id = int(start) - 1 if start and start.isdigit() else ""
        else:
            if start == 1:
                self._pagination = {}
            if start <= 10000:
                result_id = (query, kwargs.get("geo"), kwargs.get(
                    "date"), int(start) - 1, count)
                result_id = self._pagination.get(result_id, "")
            else:
                result_id = int(start) - 1
        # 1) Construct request URL.
        ...
        for x in data.get("statuses", []):
            ...
            results.append(r)
        # Twitter.search(start=result_id, count=10) takes a tweet.result_id,
        # and returns 10 results that are older than this result_id.
        ...
        # that remembers the last result_id for a "page" for a given query,
        # when called in a loop.
        #
        # Store the last result_id retrieved.
        # If search() is called again with start+1, start from this result_id.
    ...
    def trends(self, **kwargs):
        ...
        try:
            ...
            return []
        return [decode_string(x.get("name")) for x in data[0].get("trends", [])]
...
MEDIAWIKI_NAMESPACE += [space + " talk" for space in MEDIAWIKI_NAMESPACE] + ["Talk", "Special", "Media"]
...
class MediaWikiArticle(object):
    def __init__(self, title="", source="", links=None, categories=None, languages=None, disambiguation=False,
                 **kwargs):
...
class MediaWikiTable(object):
    def __init__(self, section, title="", headers=None, rows=None, source=""):
    ...
    def plaintext(self, **kwargs):
        return self.section.plaintext(self.source, **kwargs)
...
class DBPedia(SearchEngine):
    ...
    def search(self, query, type=SPARQL, start=1, count=10, sort=RELEVANCY, size=None, cached=False, **kwargs):
        ...
        # 2) Parse JSON response.
        try:
            data = URL(str(url)).download(cached=cached, timeout=30, **kwargs)
            data = json.loads(data)
...
class Flickr(SearchEngine):
    ...
    def search(self, query, type=IMAGE, start=1, count=10, sort=RELEVANCY, size=None, cached=True, **kwargs):
        ...
        kwargs.setdefault("throttle", self.throttle)
        data = url.download(cached=cached, **kwargs)
        data = xml.dom.minidom.parseString(encode_string(data))
...
class Facebook(SearchEngine):
    ...
    def search(self, query, type=SEARCH, start=1, count=10, cached=False, **kwargs):
        ...
        if type in (SEARCH, NEWS):
            max_results = 100
        elif type in (COMMENTS, LIKES):
            max_results = 1000
        elif type in (FRIENDS,):
            max_results = 10000
        else:
            raise SearchEngineTypeError
        if not query or start < 1 or count < 1:
        ...
        if type == SEARCH:
            ...
            url = URL(url, method=GET, query={
                ...
                "offset": (start - 1) * min(count, max_results),
                "limit": (start - 0) * min(count, max_results)
            })
        if type in (NEWS, FEED, COMMENTS, LIKES, FRIENDS):
            url = FACEBOOK + \
                  (decode_string(query) or "me").replace(FACEBOOK, "") + \
        ...
        kwargs.setdefault("throttle", self.throttle)
        try:
            data = URL(str(url)).download(**kwargs)
        except HTTP400BadRequest:
            raise HTTP401Authentication
        data = json.loads(data)
    ...
    def profile(self, id=None, **kwargs):
        ...
        # 1) Construct request URL.
        url = FACEBOOK + (decode_string(id or "me")).replace(FACEBOOK, "")
...
class ProductWiki(SearchEngine):
    ...
    def search(self, query, type=SEARCH, start=1, count=10, sort=RELEVANCY, size=None, cached=True, **kwargs):
        ...
        for x in (data.get("products") or [])[:count]:
            r = Result(url=None)
            r.__dict__["title"] = decode_string(x.get("title"))
            r.__dict__["text"] = decode_string(x.get("text"))
            r.__dict__["reviews"] = []
...
class Newsfeed(SearchEngine):
    ...
    def search(self, query, type=NEWS, start=1, count=10, sort=LATEST, size=SMALL, cached=True, **kwargs):
        ...
        data = feedparser.parse(encode_string(data))
        results = Results(query, query, NEWS)
        results.total = None
        for x in data["entry"][:count]:
            s = "\n\n".join([v.get("value")
...
def query(string, service=GOOGLE, **kwargs):
    ...
    if service in (GOOGLE, "google", "g"):
        engine = Google
    elif service in (YAHOO, "yahoo", "y!"):
        engine = Yahoo
    elif service in (BING, "bing"):
        engine = Bing
    elif service in (DUCKDUCKGO, "duckduckgo", "ddg"):
        engine = DuckDuckGo
    elif service in (TWITTER, "twitter", "tw"):
        engine = Twitter
    elif service in (FACEBOOK, "facebook", "fb"):
        engine = Facebook
    elif service in (WIKIPEDIA, "wikipedia", "wp"):
        engine = Wikipedia
    elif service in (WIKIA, "wikia"):
        engine = Wikia
    elif service in (DBPEDIA, "dbpedia", "dbp"):
        engine = DBPedia
    elif service in (FLICKR, "flickr"):
        engine = Flickr
    else:
...
def sort(terms=None, context="", service=GOOGLE, license=None, strict=True, prefix=False, **kwargs):
...
class Node(object):
    ...
    @property
    def children(self):
        return [self._wrap(x) for x in self._p.contents] if hasattr(self._p, "contents") else []
    ...
    def __str__(self):
        return encode_string(self.__unicode__())

    def __unicode__(self):
        return decode_string(self._p)
...
class Element(Node):
    ...
    def content(self):
        ...
        return "".join([decode_string(x) for x in self._p.contents])
...
class Document(Element):
    def __init__(self, html, **kwargs):
        ...
        Node.__init__(self, decode_string(html).strip(), type=DOCUMENT, **kwargs)
...
class Selector(object):
    ...
    def search(self, e):
        ...
        def re_search(s):
            return re.compile(r"\b%s(?=$|\s)" % s, re.I)

        a = {"id": re_search(self.id)} if self.id else {}
        ...
        if len(self.classes) == 1:
            e = list(map(Element,
                         e._p.findAll(tag, atts=dict(a, **{"class": re_search(list(self.classes)[0])}))))
...
class SelectorChain(list):
    def __init__(self, selector):
        ...
        for selector in selector.split(","):
            ...
            selector = re.sub(r" *\+ *", " +", selector)
            selector = re.sub(r"\[.*?\]",
                              lambda m: _encode_space(m.group(0)), selector)
            selector = re.sub(r":contains\(.*?\)",
                              lambda m: _encode_space(m.group(0)), selector)
            self.append([])
            for selector in selector.split(" "):
                if not selector.startswith((">", "<", "+")):
                    self[-1].append((" ", Selector(selector)))
                elif selector.startswith(">"):
                    self[-1].append((">", Selector(selector[1:])))
                elif selector.startswith("<"):
                    self[-1].append(("<", Selector(selector[1:])))
                elif selector.startswith("+"):
                    self[-1].append(("+", Selector(selector[1:])))
...
class Link(object):
    def __init__(self, url, text="", relation="", referrer=""):
        ...
        self.url, self.text, self.relation, self.referrer = \
            decode_string(url), decode_string(text), decode_string(relation), decode_string(referrer),
...
class Crawler(object):
    def __init__(self, links=None, domains=None, delay=20.0, parse=HTMLLinkParser().parse, sort=FIFO):
...
def crawl(links=None, domains=None, delay=20.0, parse=HTMLLinkParser().parse, sort=FIFO, method=DEPTH, **kwargs):
...
class DocumentParser(object):
    ...
    def _parse(self, path, *args, **kwargs):
        ...
        document."""
        return plaintext(decode_string(self.open(path).read()))
...
def parsehtml(path, **kwargs):
    """Returns the content as a Unicode string from the given .html file."""
    return plaintext(Document(path, **kwargs).body)


def parsedoc(path, doc_format=None):
    """Returns the content as a Unicode string from the given document (.html.,
    .pdf, .docx)."""
    if isinstance(path, str):
        if doc_format == "pdf" or path.endswith(".pdf"):
            return parsepdf(path)
        if doc_format == "docx" or path.endswith(".docx"):
            return parsedocx(path)
        if doc_format == "html" or path.endswith((".htm", ".html", ".xhtml")):
