...
from pattern3.text.tree import SLASH, WORD, POS, CHUNK, PNP, REL, ANCHOR, LEMMA, AND, OR
...
class Model(object):
    ...
    def __init__(self, path="", classifier=None, known=set(), unknown=set()):
        ...
        try:
            from pattern3.vector import Classifier
            from pattern3.vector import Perceptron
        except ImportError:
...
def commandline(parse=Parser().parse):
    ...
    if o.version:
        sys.path.insert(0, os.path.join(MODULE, "..", ".."))
        from pattern3 import __version__
        print(__version__)
        sys.path.pop(0)
