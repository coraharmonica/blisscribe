...
class UninstallPthEntries(object):
    def __init__(self, pth_file):
        if not os.path.isfile(pth_file):
            raise UninstallationError(
                "Cannot remove entry from nonexistent file %s" % pth_file
            )
        self.file = pth_file
        self.entries = set()
    ...
    def add(self, entry):
        ...
        # backslashes.  This is correct for entry that describe absolute
    ...
    def remove(self):
        logger.debug('Removing pth entry from %s:', self.file)
