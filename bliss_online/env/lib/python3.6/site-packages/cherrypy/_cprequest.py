...
def request_namespace(k, v):
    ...
    # Provides config entry to set request.body attrs (like
    # attempt_charsets).
    if k[:5] == 'body.':
...
def response_namespace(k, v):
    ...
    # Provides config entry to set default response headers
...
class Request(object):
    ...
    config = None
    """
    A flat dict of all configuration entry which apply to the
    current request. These entry are collected from global config,
